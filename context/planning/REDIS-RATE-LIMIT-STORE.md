...
O comentário `src/shared/http/app.ts:111` já antecipa: _"in-memory; producao sobrepoe com store Redis"_.
...
- **Escopo do ADR:** decidir se é _"Valkey só para rate-limit"_ (mínimo) ou _"Valkey como store
  compartilhado do core-api"_ (cache, sessão, rate-limit) — o segundo amortiza o custo de infra.
