...
- **CA1** — Toda rota sob `/api/v2/contracts/*` exige `requireAuth` (401 sem token; do `auth/public-api/http.ts`); mutações exigem `authorize(permission)` (403). _(diferente das rotas auth, que são públicas.)_
...
**Follow-ups (não-bloqueantes, `context/planning/`):**
