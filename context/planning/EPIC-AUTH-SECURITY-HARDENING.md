...
2. **Persistência:** tabela `auth_password_reset` (token*hash, user_id, expires_at, used_at). Prefixo `auth*\*`.
...
>
> - ✅ `CTR-AUTH-RESET-MAILER-SMTP` — **ENTREGUE (closed-green, 2026-05-30)**. `buildResetMailer(env)` fia o Nodemailer real (SMTP\_\* + `AUTH_RESET_FROM`) ou no-op seguro. Reset funciona ponta-a-ponta com SMTP.
...
> - ✅ **Store compartilhado p/ rate-limit — DECIDIDO ADIAR** (ADR-0030 Proposed, 2026-05-30): single-instance hoje → mantém in-memory (YAGNI); direção futura travada (Valkey + ioredis, store geral). Ativar antes de escalar horizontalmente. Análise: `context/planning/REDIS-RATE-LIMIT-STORE.md`.
> - ✅ `CTR-AUTH-MYSQL-INTEGRATION-VALIDATE` — **ENTREGUE (closed-green, 2026-05-30)**. MySQL do projeto subido na **3307** (compose `${MYSQL_PORT}`, sem tocar o `bemcomum-mysql` na 3306); testes de porta parametrizados. Migrations 0000-0002 aplicam + repos Drizzle de reset/lockout validados (round-trip real, 29+2 testes pass).
>
> **🎉 ÉPICO 100% CONCLUÍDO** — spec 003 inteira + todos os follow-ups (mailer real, lockout persistido, decisão do store compartilhado adiada, validação MySQL). Nada pendente.
