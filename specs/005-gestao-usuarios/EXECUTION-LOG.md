...
**Status:** ✅ concluída · **Branch:** `005-gestao-usuarios` (mergeada em `dev`) · **Atualizado:** 2026-06-08
...
- **2026-06-08** — **🏁 Fechamento da 005.** Confirmado que a branch `005-gestao-usuarios` está 100% mergeada em `dev` (`git rev-list dev...origin/005-gestao-usuarios` = 18/0) — código, schema/migrations (0002→0006), read model `UserQuery`, rotas HTTP (`/api/v1/users`, `/me`) e coleção Bruno (`api-collections/auth/`) todos presentes em `dev`. Validação de fechamento **verde**: gate W3 unit (typecheck + format:check + lint + `pnpm test` = **2454 testes, 0 fail**), `test:integration:auth` (MySQL Docker, **40/40**) e `test:e2e:bruno:auth` (**45 req / 59 testes**). No caminho: corrigidos 3 gates de regressão-zero alheios à feature mas vermelhos em `dev` — Prettier em `.github/workflows/deploy-qa.yml`, ignore de `specs/**` no `eslint.config.js` (os `contracts/*.ts` do FIN-DOCUMENTO importados hoje quebravam o type-checked lint), e `scripts/e2e-bruno-auth.sh` (rodava pasta-por-pasta perdendo o JWT entre pastas → 401; agora run único recursivo `bru run . -r`). `tasks.md` reconciliado (52/52 marcadas) e `spec.md` → Status Implementada. **Não há PR a abrir — já está em `dev`.**
