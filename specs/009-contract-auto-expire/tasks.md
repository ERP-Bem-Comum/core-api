...
- [ ] T001 Confirmar que nenhuma dependência nova é necessária (reusa Drizzle/mysql2, `node:test`, worker
      existente) e que a branch/feature está ativa (`specs/009-contract-auto-expire`).

## Phase 2 — Foundational (bloqueia as user stories)

- [ ] T002 [P] W0 RED: teste do helper de cutoff em
...
- [ ] T003 [P] W0 RED: teste de `findExpirable(cutoff)` (in-memory) em
...
- [ ] T008 [P] [US1] W0 RED: teste do use case em
...
- [ ] T009 [US1] Implementar o use case `expireDueContracts(deps)()` em
...
- [ ] T010 [US1] W0 RED: teste do tick em `tests/modules/contracts/worker/expire-scheduler.test.ts` — chama
      o use case e loga `scanned/expired/failures`; respeita o `AbortSignal` (sem timers reais).
- [ ] T011 [US1] Implementar `runExpireScheduler(deps, intervalMs)` em
      `src/modules/contracts/worker/expire-scheduler.ts` (loop com abort; loga a contagem por ciclo).
- [ ] T012 [US1] Ler `CONTRACTS_EXPIRE_SWEEP_MS` (default 3.600.000) em
      `src/modules/contracts/worker/config.ts`.
- [ ] T013 [US1] Wirar no `src/modules/contracts/worker/run.ts`: `ContractRepository` Drizzle sobre o pool
...
**Goal**: a finalização automática emite `ContractExpired` (paridade com o manual) e é idempotente.
**Independent Test**: após o sweep, o evento consta no outbox; 2ª execução = no-op.

- [ ] T014 [P] [US2] W0 RED: estender `expire-due-contracts.test.ts` — assere que `save` é chamado com
      `[ContractExpired]` (evento no outbox) e que **2ª execução** não re-expira nem duplica evento (idempotência).
- [ ] T015 [US2] Garantir no use case (T009) que o evento de `Contract.expire` é passado a `save(contract,
...
- [ ] T016 [P] [US3] W0 RED: estender `expire-due-contracts.test.ts` — `end = hoje_BRT` → permanece Active;
      `end = hoje_BRT − 1` → Expira; boundary de fuso (`02:00Z` vs `04:00Z` no mesmo dia).
- [ ] T017 [US3] Confirmar o predicado `PlainDate.isBefore(end, cutoff)` (estritamente anterior) no use case
      (T009) — ajustar se T016 falhar. Documentar que a guarda do `/end` manual NÃO muda.

## Phase 6 — Polish & Cross-Cutting

- [ ] T018 [P] Atualizar `handbook/infrastructure/05-local-server-parity-env.md` e o quickstart com
...
- [ ] T020 Mover o card `handbook/tickets/todo/CTR-CONTRACT-AUTO-EXPIRE.md` → `done/` ao fechar.
