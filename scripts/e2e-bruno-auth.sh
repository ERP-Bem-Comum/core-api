...
AUTH_DRIVER=mysql \
  ...
  CORE_API_E2E=1 \
  AUTH_SEED_JSON='{"users":[{"email":"admin.e2e@bemcomum.dev","password":"Str0ng-Passphrase-2026!","permissions":["user:list","user:read","user:create","user:update","user:activate","user:deactivate","role:read","role:create","role:update","user:assign-role"]},{"email":"bare.e2e@bemcomum.dev","password":"Str0ng-Passphrase-2026!","permissions":[]}]}' \
  PORT=3100 \
  LOG_LEVEL=warn \
