 ...
 * o carimbo Plano + Subcategoria no título MANUAL, na borda HTTP (driver `memory`).
 *
 * Espelha `financial-manual-entry.http.test.ts` (US5): mesmo wiring de auth FAKE, mesmo extrato
 * semeado com transações `Pending`. A borda passa a ACEITAR `budgetPlanRef`/`subcategoryRef` no body
 * e a ECOAR os dois no DTO de resposta — como os refs já fazem no documento (S1).
 *
 * DEVE FALHAR em W0 (pelo motivo certo):
 *  - CA2/CA6: `POST /manual-entry { budgetPlanRef, subcategoryRef }` — hoje o Zod
 *    (`manualEntryBodySchema`) NÃO conhece os campos → `z.object` DESCARTA as chaves → o use-case
 *    nunca as vê → o DTO de resposta (`manualEntryResponseSchema`) não tem os campos → `undefined`.
 *  - CA4: sem os campos, o eco `null` também é `undefined` → asserção falha.
 *  - CA5 (borda): ref malformado é descartado (não validado) → hoje devolve 201, não 400.
 *
 * Roda em `pnpm test` puro (sem MySQL) — UNIT/borda com repositórios InMemory. Regressão zero (CA8):
 * NÃO edita nenhuma suíte existente; os fluxos manual-entry/batch seguem intactos.
 *
...
// UUID v4 VÁLIDOS (grupo 3 começa com '4', grupo 4 com 8/9/a/b). Sem caractere não-hex.
...
describe('FIN-MANUAL-ENTRY-TAXONOMY — carimbo no título manual (#502 · S2)', () => {
  it('CA2/CA6: Payment com budgetPlanRef+subcategoryRef → 201 e resposta ecoa os dois', async () => {
    ...
    assert.equal(body.budgetPlanRef, BUDGET);
    assert.equal(body.subcategoryRef, SUBCATEGORY);
  });

  it('CA6: Receipt (recebimento) com os dois refs → 201 e resposta ecoa os dois', async () => {
  ...
  it('CA4: sem os refs novos → 201 e resposta ecoa null (back-compat)', async () => {
    const res = await postManualEntry(String(txIds[2]), { type: 'Payment' });
    assert.equal(res.statusCode, 201, res.body);
    const body = res.json() as ManualEntryDetail;
    assert.equal(body.budgetPlanRef, null);
    assert.equal(body.subcategoryRef, null);
  });
  ...
  it('CA5 (borda): budgetPlanRef malformado → 400', async () => {
    const res = await postManualEntry(String(txIds[3]), {
      type: 'Payment',
