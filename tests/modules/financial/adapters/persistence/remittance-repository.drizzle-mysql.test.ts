...
const build = (documentIds: readonly string[], cedente = account) => {
  ...
  const r = create({
    ...
    //
    // Esta fixture já esteve no formato do MySQL (`2026-08-11 14:00:00.000`), escrito à mão. Com
    // ele o teste passava contra MySQL real enquanto `POST /financial/remittances` falhava em
    // produção com 1292 (`Incorrect datetime value`), porque a coluna é `datetime` em
    // `mode: 'string'` e o Drizzle repassa a string crua. Um teste alimentado com dado que a
    // aplicação nunca gera não prova o caminho da aplicação — prova outro caminho.
