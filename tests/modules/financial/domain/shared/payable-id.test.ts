...
describe('PayableId — module-as-namespace (Padrão D)', () => {
  it('module is importable via `import * as PayableId` (Padrão D smoke)', () => {
    // Arrange
    const ns: Readonly<Record<string, unknown>> = PayableId;
    // Act / Assert
    assert.equal(typeof ns.generate, 'function');
    assert.equal(typeof ns.rehydrate, 'function');
  });

  it("does NOT expose a nested `PayableId` namespace-object (DON'T B§7)", () => {
    // Arrange / Act
    const ns: Readonly<Record<string, unknown>> = PayableId;
    // Assert
...
describe('PayableId — rehydrate', () => {
  it('accepts a valid v4 UUID', () => {
    // Arrange / Act
    const r = PayableId.rehydrate(VALID_V4);
    // Assert
  ...
  it('accepts uppercase hex digits (case-insensitive)', () => {
    // Arrange / Act
    const r = PayableId.rehydrate(VALID_V4_UPPER);
    // Assert
    assert.equal(isOk(r), true);
  });
  ...
  it('rejects empty string', () => {
    // Arrange / Act
    const r = PayableId.rehydrate('');
    // Assert
    assert.equal(isErr(r), true);
    if (!r.ok) assert.equal(r.error, 'payable-id-invalid');
  });

  it('rejects non-UUID string', () => {
    // Arrange / Act
    const r = PayableId.rehydrate('not-a-uuid');
    // Assert
    assert.equal(isErr(r), true);
    if (!r.ok) assert.equal(r.error, 'payable-id-invalid');
  });
  ...
  it('rejects UUID v1 (wrong version)', () => {
    // Arrange / Act
    const r = PayableId.rehydrate(V1_UUID);
    // Assert
    assert.equal(isErr(r), true);
    if (!r.ok) assert.equal(r.error, 'payable-id-invalid');
  });
