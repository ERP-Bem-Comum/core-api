...
describe('AmendmentId — module-as-namespace (Padrão D)', () => {
  it('module is importable via `import * as AmendmentId` (Padrão D smoke)', () => {
    // Arrange
    const ns: Readonly<Record<string, unknown>> = AmendmentId;
    // Act / Assert
    assert.equal(typeof ns.generate, 'function');
    assert.equal(typeof ns.rehydrate, 'function');
  });

  it("does NOT expose a nested `AmendmentId` namespace-object (DON'T B§7)", () => {
    // Arrange / Act
    const ns: Readonly<Record<string, unknown>> = AmendmentId;
    // Assert
...
describe('AmendmentId — generate', () => {
  it('returns a v4 UUID string', () => {
    // Arrange / Act
    const id = AmendmentId.generate();
    // Assert
  ...
  it('returns distinct values on consecutive calls', () => {
    // Arrange / Act
    const a = AmendmentId.generate();
    const b = AmendmentId.generate();
    // Assert
    assert.notEqual(a, b);
  });
});

describe('AmendmentId — rehydrate', () => {
  it('accepts a valid v4 UUID', () => {
    // Arrange / Act
    const r = AmendmentId.rehydrate(VALID_V4);
    // Assert
  ...
  it('accepts uppercase hex digits (case-insensitive)', () => {
    // Arrange / Act
    const r = AmendmentId.rehydrate(VALID_V4_UPPER);
    // Assert
    assert.equal(isOk(r), true);
  });
  ...
  it('rejects non-UUID string', () => {
    // Arrange / Act
    const r = AmendmentId.rehydrate('not-a-uuid');
    // Assert
    assert.equal(isErr(r), true);
    if (!r.ok) assert.equal(r.error, 'amendment-id-invalid');
  });
  ...
  it('rejects UUID v1 (wrong version)', () => {
    // Arrange / Act
    const r = AmendmentId.rehydrate(V1_UUID);
    // Assert
    assert.equal(isErr(r), true);
    if (!r.ok) assert.equal(r.error, 'amendment-id-invalid');
  });
