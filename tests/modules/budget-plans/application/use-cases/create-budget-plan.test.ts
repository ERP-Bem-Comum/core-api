...
describe('createBudgetPlan', () => {
  it('CA1: Ano+Programa válidos -> RASCUNHO v1.0 + BudgetPlanCreated + persistido', async () => {
    const deps = makeDeps();
    const r = await createBudgetPlan(deps)({ year: 2026, programRef: PROGRAM_ETI_REF });
    assert.ok(isOk(r));
  ...
  it('CA2: plano raiz duplicado (mesmo year+programa) -> budget-plan-already-exists', async () => {
    const deps = makeDeps();
    const first = await createBudgetPlan(deps)({ year: 2026, programRef: PROGRAM_ETI_REF });
    assert.ok(isOk(first));
    const dup = await createBudgetPlan(deps)({ year: 2026, programRef: PROGRAM_ETI_REF });
    assert.ok(isErr(dup));
  ...
  it('mesmo programa, ano diferente -> permitido', async () => {
    const deps = makeDeps();
    assert.ok(isOk(await createBudgetPlan(deps)({ year: 2026, programRef: PROGRAM_ETI_REF })));
    assert.ok(isOk(await createBudgetPlan(deps)({ year: 2027, programRef: PROGRAM_ETI_REF })));
  });

  it('mesmo ano, programa diferente -> permitido', async () => {
    const deps = makeDeps();
    assert.ok(isOk(await createBudgetPlan(deps)({ year: 2026, programRef: PROGRAM_ETI_REF })));
    assert.ok(isOk(await createBudgetPlan(deps)({ year: 2026, programRef: PROGRAM_PARC_REF })));
  });

  it('programa inexistente -> program-not-found', async () => {
    const r = await createBudgetPlan(makeDeps())({ year: 2026, programRef: PROGRAM_UNKNOWN_REF });
    assert.ok(isErr(r));
    assert.equal(r.error, 'program-not-found');
  });

  it('programa inativo -> program-not-active', async () => {
    const r = await createBudgetPlan(makeDeps())({ year: 2026, programRef: PROGRAM_INACTIVE_REF });
    assert.ok(isErr(r));
    assert.equal(r.error, 'program-not-active');
  });

  it('programRef malformado -> budget-plan-ref-invalid', async () => {
    const r = await createBudgetPlan(makeDeps())({ year: 2026, programRef: 'nao-e-uuid' });
    assert.ok(isErr(r));
    assert.equal(r.error, 'budget-plan-ref-invalid');
  });

  it('ano inválido -> budget-plan-invalid-year', async () => {
    const r = await createBudgetPlan(makeDeps())({ year: 1800, programRef: PROGRAM_ETI_REF });
    assert.ok(isErr(r));
