/**
 * W0 RED — REP / #590 "Fluxo de Caixa" (Slice B). Borda GET /api/v2/reports/cashflow/chart — SÉRIE
 * TEMPORAL: as mesmas Saídas (Payables) do Slice A, agora com uma DIMENSÃO DE MÊS
 * (`Installments_dueDate`). Resposta = ARRAY plano de linhas datadas (sem envelope). RED por
 * inexistência da rota/dep `listCashflowChart` no módulo `reports`.
 *
 * CA1: 200 com array de linhas datadas; shape LEGADO das 7 chaves (as 6 do Slice A + o mês).
 * CA2: RBAC — sem `fiscal-document:read` → 403.
 * CA3: contrato de saída LEGADO fechado (7 colunas por linha); `Installments_dueDate` é date.
 * CA4: os 10 filtros (AND, id-suffixed) viram o mesmo CashflowFilter repassado ao reader.
 * CA5..CA7: validação de borda (uuid/data/enum → 400).
 */
...
const chartRow = (over: Partial<CashflowChartRow> = {}): CashflowChartRow => ({
  ...
  installmentsDueDate: '2026-01-01',
...
before(async () => {
  ...
  const deps = {
    ...base,
    listCashflowChart: (filter?: CashflowFilter) => {
      lastFilter = filter;
      return Promise.resolve(
        ok([
          chartRow(),
          chartRow({ installmentsDueDate: '2026-02-01', realizedCents: 70000, expectedCents: 0 }),
          chartRow({
            categoryRef: null,
            categoryName: null,
            subcategoryRef: null,
            subcategoryName: null,
            installmentsDueDate: '2026-02-01',
            realizedCents: 0,
            expectedCents: 5000,
          }),
        ] as readonly CashflowChartRow[]),
      );
    },
  };
...
const get = (perm: string, query = ''): Promise<LightMyRequestResponse> =>
  handle.app.inject({
    method: 'GET',
    url: `/api/v2/reports/cashflow/chart${query}`,
...
describe('reports/http — GET /reports/cashflow/chart (REP · #590 · Slice B)', () => {
  it('CA1: 200 com array de linhas datadas (shape legado + Installments_dueDate)', async () => {
    ...
    assert.equal(first['Installments_dueDate'], '2026-01-01');
    // grupo sem categoria/subcategoria → refs/nomes null (degradação graciosa), mês presente.
    assert.equal(body[2]!['Category_id'], null);
    assert.equal(body[2]!['SubCategory_name'], null);
    assert.equal(body[2]!['Installments_dueDate'], '2026-02-01');
    assert.equal(body[2]!['EXPECTED'], 5000);
  });
  ...
  it('CA3: contrato de saída legado fechado (7 colunas por linha, com o mês)', async () => {
    const res = await get(READER);
    const body = res.json() as Record<string, unknown>[];
    assert.deepEqual(
  ...
  it('CA4: os 10 filtros na querystring viram o CashflowFilter (AND) repassado ao reader', async () => {
  ...
  it('CA6: status fora dos 6 (Draft/Refused/lixo) → 400', async () => {
  ...
  it('CA7: ref malformada / data inválida → 400', async () => {
    assert.equal((await get(READER, '?categoryId=not-a-uuid')).statusCode, 400);
    assert.equal((await get(READER, '?dueFrom=2026-13-99')).statusCode, 400);
  });
});
