 ...
 *
 * Molde dos writers anteriores: opt-in PARTNERS_ETL_INTEGRATION=1 + skip-guard de
 * daemon (probe gated por RUN). Sequência de negócio real: parceiros → contratos →
 * financeiro (o remap de suppliers/contratos exige as ondas anteriores no destino).
 * Fixture sintética — JAMAIS o dump de produção.
 *
...
describe(
  'FINANCIAL WRITER integration — legado efêmero → core-api destino',
  { skip: skipReason },
  () => {
    it('migra fixture (pós-parceiros+contratos): balanço, F4/F5, aprovador D11, soma', async () => {
      ...
      const contracts = await runContractsEtl({
        dumpPath: FIXTURE,
        connectionString: CORE_CONN,
        dryRun: false,
      });
