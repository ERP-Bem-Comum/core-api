\section*{Chapter 20 Group Replication}
Table of Contents
...
\subsection*{20.1 Group Replication Background}
...
\subsection*{20.1.1 Replication Technologies}
...
\subsection*{20.1.1.1 Source to Replica Replication}
...
\caption{Figure 20.1 MySQL Asynchronous Replication}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3612.jpg?height=689&width=1595&top_left_y=303&top_left_x=283}
\end{figure}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.2 MySQL Semisynchronous Replication}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3612.jpg?height=880&width=1593&top_left_y=1254&top_left_x=283}
\end{figure}
...
\subsection*{20.1.1.2 Group Replication}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.3 MySQL Group Replication Protocol}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3614.jpg?height=728&width=1589&top_left_y=312&top_left_x=287}
\end{figure}

\subsection*{20.1.2 Group Replication Use Cases}
...
\subsection*{20.1.3 Multi-Primary and Single-Primary Modes}
...
Regardless of the deployed mode, Group Replication does not handle client-side failover. That must be handled by a middleware framework such as MySQL Router 8.4, a proxy, a connector, or the application itself.

\subsection*{20.1.3.1 Single-Primary Mode}

In single-primary mode (group_replication_single_primary_mode=0N) the group has a single primary server that is set to read/write mode. All the other members in the group are set to read-only mode (with super_read_only=0N). The primary typically bootstraps the entire group. All other servers that join the group learn about the primary server and are automatically set to read-only mode.

In single-primary mode, Group Replication enforces that only a single server writes to the group, so compared to multi-primary mode, consistency checking can be less strict and DDL statements do not need to be handled with any extra care. The option group_replication_enforce_update_everywhere_checks enables or disables strict consistency checks for a group. When deploying in single-primary mode, or changing the group to single-primary mode, this system variable must be set to 0FF.
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.4 New Primary Election}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3616.jpg?height=650&width=1610&top_left_y=641&top_left_x=280}
\end{figure}
...
\section*{Primary Election Algorithm}
...
\subsection*{20.1.3.2 Multi-Primary Mode}

In multi-primary mode (group_replication_single_primary_mode=0FF) no member has a special role. Any member that is compatible with the other group members is set to read/write mode when joining the group, and can process write transactions, even if they are issued concurrently.
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.5 Client Failover}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3617.jpg?height=645&width=1604&top_left_y=1873&top_left_x=347}
\end{figure}

Group Replication is an eventual consistency system. This means that as soon as the incoming traffic slows down or stops, all group members have the same data content. While traffic is flowing,
transactions can be externalized on some members before the others, especially if some members have less write throughput than others, creating the possibility of stale reads. In multi-primary mode, slower members can also build up an excessive backlog of transactions to certify and apply, leading to a greater risk of conflicts and certification failure. To limit these issues, you can activate and tune Group Replication's flow control mechanism to minimize the difference between fast and slow members. For more information on flow control, see Section 20.7.2, "Flow Control".
...
The checks are controlled by the group_replication_enforce_update_everywhere_checks system variable. In multi-primary mode, the system variable should normally be set to 0N, but the checks can optionally be deactivated by setting the system variable to OFF. When deploying in singleprimary mode, the system variable must be set to OFF.
...
\subsection*{20.1.4 Group Replication Services}

This section introduces some of the services that Group Replication builds on.

\subsection*{20.1.4.1 Group Membership}
...
\subsection*{20.1.4.2 Failure Detection}
...
Where the network is unstable and members frequently lose and regain connection to each other in different combinations, it is theoretically possible for a group to end up marking all its members for
...
\subsection*{20.1.4.3 Fault-tolerance}

MySQL Group Replication builds on an implementation of the Paxos distributed algorithm to provide distributed coordination between servers. As such, it requires a majority of servers to be active to reach quorum and thus make a decision. This has direct impact on the number of failures the system can tolerate without compromising itself and its overall functionality. The number of servers ( n ) needed to tolerate f failures is then $\mathrm{n}=2 \times \mathrm{f}+1$.
...
\begin{tabular}{|l|l|l|}
\hline Group Size & Majority & Instant Failures Tolerated \\
\hline 1 & 1 & 0 \\
\hline 2 & 2 & 0 \\
\hline 3 & 2 & 1 \\
\hline 4 & 3 & 1 \\
...
\subsection*{20.1.4.4 Observability}
...
\subsection*{20.1.5 Group Replication Plugin Architecture}

MySQL Group Replication is a MySQL plugin and it builds on the existing MySQL replication infrastructure, taking advantage of features such as the binary log, row-based logging, and global transaction identifiers. It integrates with current MySQL frameworks, such as the performance schema
or plugin and service infrastructures. The following figure presents a block diagram depicting the overall architecture of MySQL Group Replication.

\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.6 Group Replication Plugin Block Diagram}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3622.jpg?height=1596&width=1595&top_left_y=422&top_left_x=283}
\end{figure}
...
\subsection*{20.2 Getting Started}
...
\subsection*{20.2.1 Deploying Group Replication in Single-Primary Mode}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.7 Group Architecture}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3624.jpg?height=1379&width=972&top_left_y=317&top_left_x=283}
\end{figure}
...
\subsection*{20.2.1.1 Deploying Instances for Group Replication}

The first step is to deploy at least three instances of MySQL Server, this procedure demonstrates using multiple hosts for the instances, named $\mathrm{s} 1, \mathrm{~s} 2$, and s 3 . It is assumed that MySQL Server is installed on each host (see Chapter 2, Installing MySQL). The Group Replication plugin is provided with MySQL Server 8.4; no additional software is required, although the plugin must be installed in the running MySQL server. See Section 20.2.1.1, "Deploying Instances for Group Replication"; for additional information, see Section 7.6, "MySQL Server Plugins".
...
\subsection*{20.2.1.2 Configuring an Instance for Group Replication}
...
\section*{Group Replication Settings}
...
- Configuring the group_replication_start_on_boot variable to off instructs the plugin to not start operations automatically when the server starts. This is important when setting up Group Replication as it ensures you can configure the server before manually starting the plugin. Once the
member is configured you can set group_replication_start_on_boot to on so that Group Replication starts automatically upon server boot.
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3626.jpg?height=102&width=103&top_left_y=591&top_left_x=338)
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3626.jpg?height=104&width=103&top_left_y=1964&top_left_x=338)
...
- Configuring group_replication_group_seeds sets the hostname and port of the group members which are used by the new member to establish its connection to the group. These members are called the seed members. Once the connection is established, the group membership
information is listed in the Performance Schema table replication_group_members. Usually the group_replication_group_seeds list contains the hostname:port of each of the group member's group_replication_local_address, but this is not obligatory and a subset of the group members can be chosen as seeds.
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3627.jpg?height=124&width=108&top_left_y=445&top_left_x=397)
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3627.jpg?height=115&width=111&top_left_y=1103&top_left_x=397)
...
\section*{exist. Always set group_replication_bootstrap_group=off after the first server instance comes online.}
...
\subsection*{20.2.1.3 User Credentials For Distributed Recovery}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3628.jpg?height=117&width=104&top_left_y=1681&top_left_x=301)
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3628.jpg?height=94&width=102&top_left_y=2017&top_left_x=303)
...
For more information on the security implications of each method of providing the user credentials, see Providing Replication User Credentials Securely. If you choose to provide the user credentials using a CHANGE REPLICATION SOURCE TO statement, issue the following statement on the server instance now, replacing rpl_user and password with the values used when creating the user:
```
...
\subsection*{20.2.1.4 Launching Group Replication}
...
\subsection*{20.2.1.5 Bootstrapping the Group}
...
+----------------------------+--------------------------------------+-------------+------------+-----------
| CHANNEL_NAME | MEMBER_ID | MEMBER_HOST | MEMBER_PORT | MEMBER_ST/
+----------------------------+--------------------------------------+-------------+------------+----------
...
\hline Log_name & Pos & Event_type & Server_id & End_log_pos & Info \\
\hline binlog. 000001 & 4 & Format_desc & 1 & 123 & Server ver: 8.4.9-log, Binlog ver: \\
\hline binlog. 000001 & 123 & Previous_gtids & 1 & 150 & \\
\hline binlog. 000001 & 150 & Gtid & 1 & 211 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa-a \\
\hline binlog. 000001 & 211 & Query & 1 & 270 & BEGIN \\
\hline binlog. 000001 & 270 & View_change & 1 & 369 & view_id=14724817264259180:1 \\
\hline binlog. 000001 & 369 & Query & 1 & 434 & COMMIT \\
\hline binlog. 000001 & 434 & Gtid & 1 & 495 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa-a \\
\hline binlog. 000001 & 495 & Query & 1 & 585 & CREATE DATABASE test \\
\hline binlog. 000001 & 585 & Gtid & 1 & 646 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa-a \\
\hline binlog. 000001 & 646 & Query & 1 & 770 & use ˋtestˋ; CREATE TABLE t1 (c1 INT \\
\hline binlog. 000001 & 770 & Gtid & 1 & 831 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa-a \\
\hline binlog. 000001 & 831 & Query & 1 & 899 & BEGIN \\
\hline binlog. 000001 & 899 & Table_map & 1 & 942 & table_id: 108 (test.t1) \\
\hline binlog. 000001 & 942 & Write_rows & 1 & 984 & table_id: 108 flags: STMT_END_F \\
\hline binlog. 000001 & 984 & Xid & 1 & 1011 & COMMIT /* xid=38 */ \\
\hline
\end{tabular}
```
...
\subsection*{20.2.1.6 Adding Instances to the Group}
...
\section*{Adding a Second Instance}

In order to add a second instance, server s 2 , first create the configuration file for it. The configuration is similar to the one used for server s1, except for things such as the server_id.
```
[mysqld]
#
...
If you are using the caching SHA-2 authentication plugin (the default), see Replication User With The Caching SHA-2 Authentication Plugin.
...
\begin{tabular}{|l|l|l|l|l|l|}
\hline Log_name & Pos & Event_type & Server_id & End_log_pos & Info \\
\hline binlog. 000001 & 4 & Format_desc & 2 & 123 & Server ver: 8.4.9-log, Binlog ver: \\
\hline binlog. 000001 & 123 & Previous_gtids & 2 & 150 & \\
\hline binlog. 000001 & 150 & Gtid & 1 & 211 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 211 & Query & 1 & 270 & BEGIN \\
\hline binlog. 000001 & 270 & View_change & 1 & 369 & view_id=14724832985483517:1 \\
\hline binlog. 000001 & 369 & Query & 1 & 434 & COMMIT \\
\hline binlog. 000001 & 434 & Gtid & 1 & 495 & SET @@SESSION.GTID_NEXT= 'aaaaaaaaa- \\
\hline binlog. 000001 & 495 & Query & 1 & 585 & CREATE DATABASE test \\
\hline binlog. 000001 & 585 & Gtid & 1 & 646 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 646 & Query & 1 & 770 & use ˋtestˋ; CREATE TABLE t1 (c1 INT \\
\hline binlog. 000001 & 770 & Gtid & 1 & 831 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 831 & Query & 1 & 890 & BEGIN \\
\hline binlog. 000001 & 890 & Table_map & 1 & 933 & table_id: 108 (test.t1) \\
\hline binlog. 000001 & 933 & Write_rows & 1 & 975 & table_id: 108 flags: STMT_END_F \\
\hline binlog. 000001 & 975 & Xid & 1 & 1002 & COMMIT /* xid=30 */ \\
\hline binlog. 000001 & 1002 & Gtid & 1 & 1063 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 1063 & Query & 1 & 1122 & BEGIN \\
\hline binlog. 000001 & 1122 & View_change & 1 & 1261 & view_id=14724832985483517:2 \\
\hline binlog. 000001 & 1261 & Query & 1 & 1326 & COMMIT \\
\hline
\end{tabular}
```


As seen above, the second server has been added to the group and it has replicated the changes from server s1 automatically. In other words, the transactions applied on s1 up to the point in time that s2 joined the group have been replicated to s 2 .

\section*{Adding Additional Instances}

Adding additional instances to the group is essentially the same sequence of steps as adding the second server, except that the configuration has to be changed as it had to be for server s 2 . To summarise the required operations:
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3634.jpg?height=237&width=1780&top_left_y=2126&top_left_x=287)
...
\begin{tabular}{|l|l|l|l|l|l|}
\hline Log_name & Pos & Event_type & Server_id & End_log_pos & Info \\
\hline binlog. 000001 & 4 & Format_desc & 3 & 123 & Server ver: 8.4.9-log, Binlog ver: \\
\hline binlog. 000001 & 123 & Previous_gtids & 3 & 150 & \\
\hline binlog. 000001 & 150 & Gtid & 1 & 211 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 211 & Query & 1 & 270 & BEGIN \\
\hline binlog. 000001 & 270 & View_change & 1 & 369 & view_id=14724832985483517:1 \\
\hline binlog. 000001 & 369 & Query & 1 & 434 & COMMIT \\
\hline binlog. 000001 & 434 & Gtid & 1 & 495 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 495 & Query & 1 & 585 & CREATE DATABASE test \\
\hline binlog. 000001 & 585 & Gtid & 1 & 646 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 646 & Query & 1 & 770 & use ˋtestˋ; CREATE TABLE t1 (c1 INT \\
\hline binlog. 000001 & 770 & Gtid & 1 & 831 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
...
\hline binlog. 000001 & 890 & Table_map & 1 & 933 & table_id: 108 (test.t1) \\
\hline binlog. 000001 & 933 & Write_rows & 1 & 975 & table_id: 108 flags: STMT_END_F \\
\hline binlog. 000001 & 975 & Xid & 1 & 1002 & COMMIT /* xid=29 */ \\
\hline binlog. 000001 & 1002 & Gtid & 1 & 1063 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 1063 & Query & 1 & 1122 & BEGIN \\
\hline binlog. 000001 & 1122 & View_change & 1 & 1261 & view_id=14724832985483517:2 \\
\hline binlog. 000001 & 1261 & Query & 1 & 1326 & COMMIT \\
\hline binlog. 000001 & 1326 & Gtid & 1 & 1387 & SET @@SESSION.GTID_NEXT= 'aaaaaaaa- \\
\hline binlog. 000001 & 1387 & Query & 1 & 1446 & BEGIN \\
\hline binlog. 000001 & 1446 & View_change & 1 & 1585 & view_id=14724832985483517:3 \\
\hline binlog. 000001 & 1585 & Query & 1 & 1650 & COMMIT \\
\hline
\end{tabular}
```


\subsection*{20.2.2 Deploying Group Replication Locally}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3635.jpg?height=108&width=104&top_left_y=1784&top_left_x=365)
...
\section*{Configuration of Local Group Replication Members}
...
\section*{Note \\ The non-default port of 24801 is used because in this tutorial the three server instances use the same hostname. In a setup with three different machines this would not be required.}
...
\subsection*{20.3 Requirements and Limitations}

This section lists and explains the requirements and limitations of Group Replication.

\subsection*{20.3.1 Group Replication Requirements}
- Infrastructure
...
\section*{Server Instance Configuration}

The following options must be configured as shown on server instances that are members of a group.
- Unique Server Identifier. Use the server_id system variable to configure the server with a unique server ID, as required for all servers in replication topologies. The server ID must be a positive integer between 1 and $\left(2^{32}\right)-1$, and it must be different from every other server ID in use by any other server in the replication topology.
- Binary Log Active. In MySQL 8.4, binary logging is enabled by default. You can optionally specify the names of the binary log files using --log-bin[=log_file_name]. Group Replication
...
- Multithreaded Appliers. Group Replication members can be configured as multithreaded replicas, enabling transactions to be applied in parallel. All replicas are configured as multithreaded by default. A nonzero value for replica_parallel_workers enables the multithreaded applier on the member. The default is 4 parallel applier threads; up to 1024 parallel applier threads can be specified. For a multithreaded replica, the following is also required:
replica_preserve_commit_ordris setting is required to ensure that the final commit of parallel transactions is in the same order as the original transactions. Group Replication relies on consistency mechanisms built around the guarantee that all participating members receive and apply committed transactions in the same order.

Setting replica_parallel_workers=0 disables parallel execution and gives the replica a single applier thread and no coordinator thread. With that setting, the replica_parallel_type and replica_preserve_commit_order options have no effect and are ignored. If parallel execution is disabled when GTIDs are in use on a replica, the replica actually uses one parallel worker, to take advantage of the method for retrying transactions without accessing the file positions. However, this behavior does not change anything for the user.
...
When detached XA transaction support is enabled (xa_detach_on_prepare $=0 \mathrm{~N}$ ) it is possible for any connection to this server to list (using XA RECOVER), roll back, or commit any prepared XA transaction. In addition, you cannot use temporary tables within detached XA transactions.
...
\subsection*{20.3.2 Group Replication Limitations}
- Limit on Group Size
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3639.jpg?height=127&width=90&top_left_y=1128&top_left_x=374)
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3639.jpg?height=126&width=108&top_left_y=1649&top_left_x=397)
...
- Binary Log Checksums. Group Replication in MySQL 8.4 supports checksums, so group members may use the default setting binlog_checksum=CRC32. The setting for binlog_checksum does not have to be the same for all members of a group.
...
\section*{Limits on Transaction Size}
...
\subsection*{20.4 Monitoring Group Replication}
...
- replication_group_communication_information: See Section 29.12.11.10, "The replication_group_communication_information Table".
...
\subsection*{20.4.2 Group Replication Server States}
...
\subsection*{20.4.3 The replication_group_members Table}
...
\subsection*{20.4.4 The replication_group_member_stats Table}
...
\subsection*{20.5 Group Replication Operations}

This section explains common operations for managing groups.

\subsection*{20.5.1 Configuring an Online Group}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3645.jpg?height=100&width=104&top_left_y=2416&top_left_x=365)
...
To use the functions, connect to a member of the running group and invoke the function with the SELECT statement. The Group Replication plugin processes the action and its parameters and the
coordinator sends it to all members which are visible to the member where you invoked the function. If the action is accepted, all members execute the action and send a termination message when completed. Once all members declare the action as finished, the invoking member returns the result to the client.
...
\subsection*{20.5.1.1 Changing the Primary}
...
\subsection*{20.5.1.2 Changing the Group Mode}
...
\section*{Changing to Single-Primary Mode}
...
When you change to single-primary mode, strict consistency checks are also disabled on all group members, as required in single-primary mode (group_replication_enforce_update_everywhere_checks=0FF).
...
\section*{Changing to Multi-Primary Mode}
...
\footnotetext{
SELECT event_name, work_completed, work_estimated FROM performance_schema.events_stages_current WHERE
}
```
...
\subsection*{20.5.1.3 Using Group Replication Group Write Consensus}
...
\section*{Inspecting a Group's Write Concurrency}
...
\section*{Configuring a Group's Write Concurrency}
...
\subsection*{20.5.1.4 Setting a Group's Communication Protocol Version}
...
If two members attempt to join in the same membership change event, they can only join if the communication protocol version for both members is already compatible with the group's
communication protocol version. Members with different communication protocol versions from the group must join in isolation. For example:
...
\subsection*{20.5.1.5 Configuring Member Actions}
...
The member action mysql_disable_super_read_only_if_primary can be set so that a group in single-primary mode stay in super read-only mode when a new primary is elected; this means that
...
b. Edit the MySQL Server configuration file (typically named my.cnf on Linux and Unix systems, or my.ini on Windows systems) and set the system variable group_replication_start_on_boot=0FF. This setting prevents Group Replication from starting when MySQL Server is started, which is the default.
...
fully shut down and not yet bootstrapped. If you take that approach, do not set group_replication_bootstrap_group=ON on any server at this stage.
...
It is important not to store the setting group_replication_bootstrap_group=0N in the configuration file, otherwise when the server is restarted again, a second group with the same name is set up.
...
7. When the members have rejoined the group, if you edited their configuration files to set group_replication_start_on_boot=0FF, you can edit them again to set 0N (or remove the system variable, since 0 N is the default).

\subsection*{20.5.3 Transaction Consistency Guarantees}
...
\subsection*{20.5.3.1 Understanding Transaction Consistency Guarantees}
...
\section*{Consistency Guarantees and Primary Failover}
...
With the first approach, the group takes the minimum time possible to secure a stable group membership after a primary failure by electing a new primary and then allowing data access immediately while it is still applying any possible backlog from the old primary. Write consistency is ensured, but reads can temporarily retrieve stale data while the new primary applies the backlog. For example, if client C 1 wrote $\mathrm{A}=2$ WHERE $\mathrm{A}=1$ on the old primary just before its failure, when client C 1 is reconnected to the new primary it could potentially read $\mathrm{A}=1$ until the new primary applies its backlog and catches up with the state of the old primary before it left the group.
...
Any alternative ensures that in the situation described for client C 2 would always read $\mathrm{B}=2$ even if immediately connected to a secondary. Each alternative has its advantages and disadvantages, which are directly related to your system workload. The following examples describe different types of workloads and advise which point of synchronization is appropriate.
...
\subsection*{20.5.3.2 Configuring Transaction Consistency Guarantees}
...
\section*{- BEFORE_ON_PRIMARY_FAILOVER}
...
\section*{How to Choose a Consistency Level}
...
\section*{Impacts of Consistency Levels}
...
The BEFORE consistency level, apart from being ordered on the transaction stream, only impacts on the local member. That is, it does not require coordination with the other members and does not have
repercussions on their transactions. In other words, BEFORE only impacts the transactions on which it is used.
...
\section*{Impact of Consistency on Primary Election}
...
\section*{Permitted Queries Under Consistency Rules}
...
- SET statements
- D0 statements that do not use tables or loadable functions
- EMPTY statements
- USE statements
...
\subsection*{20.5.4 Distributed Recovery}
...
\subsection*{20.5.4.1 Connections for Distributed Recovery}
...
> Server's report_host system variable to configure a unique host name to be externalized by each of the servers.
...
\section*{Selecting addresses for distributed recovery endpoints}
...
client connection as one of a number of distributed recovery endpoints, you must include it explicitly in the list specified by group_replication_advertise_recovery_endpoints. You can put it in the last place so that it acts as a last resort for connection.
...
\section*{Compression for Distributed Recovery}
...
\section*{Replication User for Distributed Recovery}
...
\section*{SSL and Authentication for Distributed Recovery}
...
\section*{Prerequisites for Cloning}
...
\section*{Cloning for Other Purposes}

Group Replication initiates and manages cloning operations for distributed recovery. Group members that have been set up to support cloning may also participate in cloning operations that a user initiates
...
\section*{Sleep Interval for Connection Attempts}
...
\section*{Marking the Joining Member Online}
...
\subsection*{20.5.4.4 Fault Tolerance for Distributed Recovery}
...
\subsection*{20.5.4.5 How Distributed Recovery Works}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.8 Stable Group}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3669.jpg?height=864&width=1587&top_left_y=973&top_left_x=346}
\end{figure}
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3669.jpg?height=269&width=540&top_left_y=1950&top_left_x=349)

\section*{View Change: a Member Joins}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.9 A Member Joins}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3670.jpg?height=1013&width=1596&top_left_y=317&top_left_x=280}
\end{figure}
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3670.jpg?height=287&width=584&top_left_y=1379&top_left_x=287)

\section*{State Transfer: Catching Up}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.10 State Transfer: Catching Up}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3671.jpg?height=994&width=1586&top_left_y=315&top_left_x=349}
\end{figure}
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3671.jpg?height=280&width=570&top_left_y=1379&top_left_x=349)
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.11 Queued Transactions}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3672.jpg?height=1198&width=1588&top_left_y=315&top_left_x=283}
\end{figure}

Group Transactions are cached while recovering
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3672.jpg?height=314&width=639&top_left_y=1676&top_left_x=287)

\section*{Finish: Caught Up}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.12 Instance Online}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3673.jpg?height=928&width=1595&top_left_y=420&top_left_x=347}
\end{figure}
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3673.jpg?height=266&width=542&top_left_y=1395&top_left_x=349)

\subsection*{20.5.5 Support For IPv6 And For Mixed IPv6 And IPv4 Groups}
...
\subsection*{20.5.6 Using MySQL Enterprise Backup with Group Replication}
...
\section*{Backing up a Group Replication Member Using MySQL Enterprise Backup}
...
Consider the following group with three members, $\mathrm{s} 1, \mathrm{~s} 2$, and s 3 , running on hosts with the same names:
```
...
```
s2> mysqlbackup --defaults-file=/etc/my.cnf --backup-image=/backups/my.mbi_ˋdate +%d%m_%H%Mˋ \
    --backup-dir=/backups/backup_ˋdate +%d%m_%H%Mˋ --user=root -p \
    --host=127.0.0.1 backup-to-image
```


\section*{Restoring a Failed Member}
...
1. Copy the backup of s 2 onto the host for s3. The exact way to copy the backup depends on the operating system and tools available to you. In this example, we assume the hosts are both Linux servers and use SCP to copy the files between them:
```
s2/backups> scp my.mbi_2206_1429 s3:/backups
```

2. Restore the backup. Connect to the target host (the host for s 3 in this case), and restore the backup using MySQL Enterprise Backup. Here are the steps:
...
```
mysql> SELECT member_host, member_port, member_state FROM performance_schema.replication_group_membe
+--------------+-------------+--------------+
| member_host | member_port | member_state |
+--------------+--------------+--------------+
| s1 | 3306 | ONLINE |
...
This shows that s 3 is applying transactions to catch up with the group. Once it has caught up with the rest of the group, its member_state changes to ONLINE:
```
mysql> SELECT member_host, member_port, member_state FROM performance_schema.replication_group_membe
+--------------+-------------+--------------+
| member_host | member_port | member_state |
...
\section*{Rebuild the Failed Member to Rejoin as a New Member}
...
If the system variables innodb_data_home_dir, innodb_log_group_home_dir, and innodb_undo_directory point to any directories other than the data directory, they should also be made empty; otherwise, the restore operation fails.
d. Restore the backup of s 2 onto the host of s 3 . With this approach, we are rebuilding s 3 as a new member, for which we do not need or do not want to use the old binary and relay logs in the backup; therefore, if these logs have been included in your backup, exclude them using the --skip-binlog and --skip-relaylog options:
```
s3> mysqlbackup --defaults-file=/etc/my.cnf \
    --datadir=/var/lib/mysql \
    --backup-image=/backups/my.mbi_2206_1429 \
    --backup-dir=/tmp/restore_ˋdate +%d%m_%H%Mˋ \
...
Restoring a Primary Member. If the restored member is a primary in the group, care must be taken to prevent writes to the restored database during the Group Replication distributed recovery process.
...
```
group_replication_start_on_boot=0FF
super_read_only=ON
event_scheduler=0FF
```
...
\subsection*{20.6 Group Replication Security}
...
\subsection*{20.6.1 Communication Stack for Connection Security Management}
...
\section*{Group Replication Requirements For The MySQL Communication Stack}
...
- The require_secure_transport server system variable value should be the same on all the group members, to avoid potential conflicts. If group_replication_ssl_mode is set to REQUIRED, VERIFY_CA, or VERIFY_IDENTITY, use require_secure_transport=0N. If group_replication_ssl_mode is set to DISABLED, use require_secure_transport=0FF.
...
This means that although the value of the system variable can be changed while Group Replication is running, and takes effect after you restart Group Replication on the group member, the member still cannot rejoin the group until the setting has been changed on all the members. You must therefore stop Group Replication on all of the members and change the value of the system variable on them all
before you can restart the group. Because all of the members are stopped, a full reboot of the group (a bootstrap by a server with group_replication_bootstrap_group=ON) is required in order for the value change to take effect. You can make the other required changes to settings on the group members while they are stopped.
...
- When you move back to the XCom communication stack, the settings specified by group_replication_recovery_use_ssl and the other group_replication_recovery_* system variables are not used to secure group communications. Instead, the Group Replication system
6. To restart the group, follow the process in Section 20.5.2, "Restarting a Group", which explains how to safely bootstrap a group where transactions have been executed and certified. A bootstrap by a server with group_replication_bootstrap_group=0N is necessary to change the communication stack, because all of the members must be shut down.
...
\subsection*{20.6.2 Securing Group Communication Connections with Secure Socket Layer (SSL)}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 20.1 group_replication_ssl_mode configuration values}
\begin{tabular}{|l|l|}
\hline Value & Description \\
\hline DISABLED & Establish an unencrypted connection (the default). \\
\hline REQUIRED & Establish a secure connection if the server supports secure connections. \\
\hline VERIFY_CA & Like REQUIRED, but additionally verify the server TLS certificate against the configured Certificate Authority (CA) certificates. \\
\hline VERIFY_IDENTITY & Like VERIFY_CA, but additionally verify that the server certificate matches the host to which the connection is attempted. \\
\hline
\end{tabular}
\end{table}
...
\section*{When using the XCom communication stack}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 20.2 SSL Options}
\begin{tabular}{|l|l|}
\hline Server Configuration & Description \\
\hline ssl_key & \begin{tabular}{l} 
...
\begin{tabular}{|l|l|}
\hline Server Configuration & Description \\
\hline & key. On the server side, this is the server private key. \\
\hline ssl_cert & The path name of the SSL public key certificate file in PEM format. On the client side, this is the client public key certificate. On the server side, this is the server public key certificate. \\
\hline ssl_ca & The path name of the Certificate Authority (CA) certificate file in PEM format. \\
\hline ssl_capath & The path name of the directory that contains trusted SSL certificate authority (CA) certificate files in PEM format. \\
\hline ssl_crl & The path name of the file containing certificate revocation lists in PEM format. \\
\hline ssl_crlpath & The path name of the directory that contains certificate revocation list files in PEM format. \\
\hline ssl_cipher & A list of permissible ciphers for encrypted connections. \\
\hline tls_version & A list of the TLS protocols the server permits for encrypted connections. \\
\hline tls_ciphersuites & Which TLSv1.3 ciphersuites the server permits for encrypted connections. \\
\hline
\end{tabular}
...
(group_replication_communication_stack=MYSQL): The security settings for distributed recovery of the group are applied to the normal communications between group members. See Section 20.6.3, "Securing Distributed Recovery Connections" on how to configure the security settings.

\subsection*{20.6.3 Securing Distributed Recovery Connections}

\section*{Important}

When using the MySQL communication stack (group_replication_communication_stack=MYSQL) AND secure connections between members (group_replication_ssl_mode is not set to DISABLED), the security settings discussed in this section are applied not

Ijust to distributed recovery connections, but to group communications between members in general.
...
\subsection*{20.6.3.1 Secure User Credentials for Distributed Recovery}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3686.jpg?height=99&width=104&top_left_y=1626&top_left_x=301)
...
\section*{Replication User With The Caching SHA-2 Authentication Plugin}
...
The less secure approach is to set group_replication_recovery_get_public_key=0N on donors so that they provide the public key of the replication user account to joining members. There is no way to verify the identity of a server, therefore only set
group_replication_recovery_get_public_key=0N when you are sure there is no risk of server identity being compromised, for example by a man-in-the-middle attack.

\section*{Replication User With SSL}
...
\section*{Providing Replication User Credentials Securely}

To supply the user credentials for the replication user, you can set them permanently as the credentials for the group_replication_recovery channel, using a CHANGE REPLICATION SOURCE TO statement. Alternatively, you can specify them in the START GROUP_REPLICATION statement each time Group Replication is started. User credentials specified in START GROUP_REPLICATION take precedence over any user credentials that have been set using a CHANGE REPLICATION SOURCE T0 statement.
...
If you switch to using START GROUP_REPLICATION to specify user credentials on a server that previously supplied the credentials using CHANGE

IREPLICATION SOURCE TO, you must complete the following steps to get the security benefits of this change.
...
\subsection*{20.6.3.2 Secure Socket Layer (SSL) Connections for Distributed Recovery}
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3688.jpg?height=99&width=106&top_left_y=1334&top_left_x=301)
...
\subsection*{20.7 Group Replication Performance and Troubleshooting}
...
\subsection*{20.7.1 Fine Tuning the Group Communication Thread}
...
\subsection*{20.7.2 Flow Control}

MySQL Group Replication ensures that a transaction commits only after a majority of the members in a group have received it and agreed on the relative order amongst all transactions sent concurrently. This approach works well if the total number of writes to the group does not exceed the write capacity of any member in the group. If it does, and some members have less write throughput than othersparticularly less than the writer members-these members may start lagging behind the writers.
...
\subsection*{20.7.2.1 Probes and Statistics}
...
\subsection*{20.7.2.2 Group Replication Throttling}
...
\subsection*{20.7.3 Single Consensus Leader}
...
- The group_replication_paxos_single_leader system variable must be set to 0N. With the default setting 0FF, the behavior is disabled. You must carry out a full reboot of the replication group (bootstrap) for Group Replication to pick up a change to this setting.
...
The Performance Schema table replication_group_communication_information shows the current preferred and actual consensus leader, with the preferred leader being Group Replication's choice, and the actual leader being the one selected by the group communication engine.
...
\subsection*{20.7.4 Message Compression}
...
The default compression threshold is 1000000 bytes. You could use the following statements to increase the compression threshold to 2 MB , for example:
```
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.13 Compression Support}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3696.jpg?height=1639&width=1591&top_left_y=826&top_left_x=285}
\end{figure}

When network bandwidth is a bottleneck, message compression can provide up to $30-40 \%$ throughput improvement at the group communication level. This is especially important within the context of large groups of servers under load. The TCP peer-to-peer nature of the interconnections between $N$ participants in the group makes the sender send the same amount of data $N$ times. Furthermore, binary
logs are likely to exhibit a high compression ratio. This makes compression a compelling feature for Group Replication workloads that contain large transactions.

\subsection*{20.7.5 Message Fragmentation}
...
The system variable group_replication_communication_max_message_size specifies a maximum message size for Group Replication communications, above which messages are fragmented. The default maximum message size is 10485760 bytes $(10 \mathrm{MiB})$. The greatest permitted value is the same as the maximum value of the replica_max_allowed_packet system variable ( 1 GB ). group_replication_communication_max_message_size must be less than replica_max_allowed_packet because the applier thread cannot handle message fragments larger than the maximum permitted packet size. To switch off fragmentation, specify a zero value for group_replication_communication_max_message_size.
...
\subsection*{20.7.6 XCom Cache Management}
...
A cache size limit can be set for XCom's message cache using the group_replication_message_cache_size system variable. If the cache size limit is reached, XCom removes the oldest entries that have been decided and delivered. The same cache size limit
...
\subsection*{20.7.6.1 Increasing the cache size}
...
\subsection*{20.7.6.2 Reducing the cache size}
...
\subsection*{20.7.7 Responses to Failure Detection and Network Partitioning}
...
\subsection*{20.7.7.1 Expel Timeout}
...
\subsection*{20.7.7.2 Unreachable Majority Timeout}
...
\subsection*{20.7.7.3 Auto-Rejoin}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 20.3 Exit actions in Group Replication failure situations}
\begin{tabular}{|l|l|l|}
\hline Failure situation & Group Replication started with START GROUP_REPLICATION & Group Replication started with group_replication_start_on_boot $=0 \mathrm{~N}$ \\
\hline Member fails local configuration check & super_read_only and offline_mode unchanged & super_read_only and offline_mode unchanged \\
\hline Mismatch between joining member and group configuration & \begin{tabular}{l}
MySQL continues running \\
Set super_read_only=0N at startup to prevent updates
\end{tabular} & \begin{tabular}{l}
MySQL continues running \\
Set super_read_only=ON at startup to prevent updates (Important)
\end{tabular} \\
\hline Applier error on member & super_read_only set to 0N & super_read_only set to 0N \\
\hline Distributed recovery not possible & OR & OR \\
\hline Group configuration change error & offline_mode and super_read_only set to 0N & offline_mode and super_read_only set to ON \\
\hline Primary election error & OR & OR \\
\hline Member expelled from group & MySQL shuts down & MySQL shuts down \\
\hline Out of auto-rejoin attempts & & \\
\hline
\end{tabular}
\end{table}

\subsection*{20.7.8 Handling a Network Partition and Loss of Quorum}
...
\section*{Note \\ Loss of quorum is by itself a side-effect of bad planning. Plan the group size for the number of expected failures (regardless whether they are consecutive, happen all at once or are sporadic).}
...
\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.14 Losing Quorum}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3706.jpg?height=1614&width=972&top_left_y=303&top_left_x=283}
\end{figure}
...
The table shows that s1 is now in a group that has no means of progressing without external intervention, because a majority of the servers are unreachable. In this particular case, the group membership list needs to be reset to allow the system to proceed, which is explained in this section. Alternatively, you could also choose to stop Group Replication on s1 and s2 (or stop completely s1 and $\mathrm{s} 2)$, figure out what happened with $\mathrm{s} 3, \mathrm{~s} 4$ and s 5 and then restart Group Replication (or the servers).

\section*{Unblocking a Partition}

Group replication enables you to reset the group membership list by forcing a specific configuration. For instance in the case above, where s1 and s2 are the only servers online, you could choose to force a membership configuration consisting of only s 1 and s 2 . This requires checking some information about s1 and s2 and then using the group_replication_force_members variable.

\begin{figure}
\captionsetup{labelformat=empty}
\caption{Figure 20.15 Forcing a New Membership}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3708.jpg?height=1614&width=972&top_left_y=303&top_left_x=283}
\end{figure}

Suppose that you are back in the situation where s 1 and s 2 are the only servers left in the group. Servers $\mathrm{s} 3, \mathrm{~s} 4$ and s 5 have left the group unexpectedly. To make servers s 1 and s 2 continue, you want to force a membership configuration that contains only s1 and s2.
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3708.jpg?height=108&width=108&top_left_y=2124&top_left_x=301)
...
\end{tabular} & UNREACHABLE ONLINE ONLINE UNREACHABLE UNREACHABLE \\
\hline
\end{tabular}
```


The first thing to do is to check what is the local address (group communication identifier) for s1 and s2. Log in to s 1 and s 2 and get that information as follows.
```
...
\hline MEMBER_ID & MEMBER_STATE \\
\hline \begin{tabular}{l}
b5ffe505-4ab6-11e6-b04b-28b2bd168d07 \\
b60907e7-4ab6-11e6-afb7-28b2bd168d07
\end{tabular} & \begin{tabular}{l}
ONLINE \\
ONLINE
\end{tabular} \\
...
\subsection*{20.7.9 Monitoring Group Replication Memory Usage with Performance Schema Memory Instrumentation}
...
\section*{Performance Schema Group Replication instruments memory allocation for Group Replication.}
...
\subsection*{20.7.9.1 Enabling or Disabling Group Replication Instrumentation}
...
\subsection*{20.7.9.2 Example Queries}
...
\section*{Memory Used to Capture Transactions}
...
\section*{Memory Used to Broadcast Transactions}
...
\section*{Total Memory Used in Group Replication}
...
\section*{Memory Used in Certification}
...
\section*{Memory Used in Replication Pipeline}
...
\section*{Memory Used in Consistency}
...
SELECT * FROM (
        ...
        FROM performance_schema.memory_summary_global_by_event_name
        GROUP BY (CASE
                        WHEN EVENT_NAME = 'memory/group_rpl/consistent_members_that_must_prepare_transa
                        THEN 'memory/group_rpl/consistency'
                        WHEN EVENT_NAME = 'memory/group_rpl/consistent_transactions'
                        THEN 'memory/group_rpl/consistency'
                        WHEN EVENT_NAME = 'memory/group_rpl/consistent_transactions_prepared'
...
\section*{Memory Used in Delivery Message Service}
...
\section*{Memory Used to Broadcast and Receive Transactions}
...
\subsection*{20.8 Upgrading Group Replication}
...
\subsection*{20.8.1 Combining Different Member Versions in a Group}

Group Replication is versioned according to the MySQL Server version that the Group Replication plugin was bundled with. For example, if a member is running MySQL 8.4.9 then that is the version of the Group Replication plugin. To check the version of MySQL Server on a group member issue:
```
...
\subsection*{20.8.1.1 Member Versions During Upgrades}
...
To deal with a problem situation, for example if you have to roll back an upgrade to a previous major version or add extra capacity to a group in an emergency, it is possible to allow a member to join an online group from a version that otherwise couldn't. The deprecated Group Replication system variable group_replication_allow_local_lower_version_join can be used in such situations to override the normal compatibility policies.
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3720.jpg?height=104&width=108&top_left_y=264&top_left_x=301)
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3720.jpg?height=108&width=95&top_left_y=653&top_left_x=310)

\section*{Note}
group_replication_allow_local_lower_version_join is deprecated and subject to removal in a future version of MySQL. Setting it generates a ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT warning.

\subsection*{20.8.1.2 Group Replication Communication Protocol Version}
...
\subsection*{20.8.2 Group Replication Offline Upgrade}
...
\subsection*{20.8.3 Group Replication Online Upgrade}
...
\subsection*{20.8.3.1 Online Upgrade Considerations}
...
\subsection*{20.8.3.2 Upgrading a Group Replication Member}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3721.jpg?height=117&width=103&top_left_y=1430&top_left_x=402)
...
\subsection*{20.8.3.3 Group Replication Online Upgrade Methods}

Choose one of the following methods of upgrading a Group Replication group:

\section*{Rolling In-Group Upgrade}
...
\section*{Rolling Migration Upgrade}
...
\section*{Rolling Duplication Upgrade}
...
\subsection*{20.8.3.4 Group Replication Upgrade with mysqlbackup}
...
Repeat this process to create a suitable number of new instances, for example to be able to handle a failover. Then join the instances to a group based on the Section 20.8.3.3, "Group Replication Online Upgrade Methods".ˋ

\subsection*{20.9 Group Replication Variables}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 20.4 Group Replication Variable and Option Summary}
\begin{tabular}{|l|l|l|l|l|l|}
\hline Name & Option File & System Var & Status Var & Var Scope & Dynamic \\
\hline Gr_all_consensus_proposals & count & & Yes & Both & No \\
\hline Gr_all_consensus_time_sum & & & Yes & Both & No \\
\hline Gr_certification_garbage_coll & ector_count & & Yes & Both & No \\
\hline Gr_certification_garbage_colle & ector_time_sum & & Yes & Both & No \\
\hline Gr_consensus_bytes_received_sum & & & Yes & Both & No \\
\hline Gr_consensus_bytes_sent_sum & & & Yes & Both & No \\
\hline Gr_control_messages_sent_bytes_sum & & & Yes & Both & No \\
\hline Gr_control_messages_sent_count & & & Yes & Both & No \\
\hline Gr_control_messages_sent_r & roundtrip_time & sum & Yes & Both & No \\
\hline Gr_data_messages_sent_byt & tes_sum & & Yes & Both & No \\
\hline Gr_data_messages_sent_cou & unt & & Yes & Both & No \\
\hline Gr_data_messages_sent_rou & undtrip_time_sum & & Yes & Both & No \\
\hline Gr_empty_consensus_proposals_count & & & Yes & Both & No \\
\hline Gr_extended_consensus_count & & & Yes & Both & No \\
\hline Gr_flow_control_throttle_active_count & & & Yes & Global & No \\
\hline Gr_flow_control_throttle_count & & & Yes & Global & No \\
\hline Gr_flow_control_throttle_last & throttle_timesta & & Yes & Global & No \\
\hline Gr_flow_control_throttle_time & _sum & & Yes & Global & No \\
\hline Gr_last_consensus_end_time & estamp & & Yes & Both & No \\
\hline
\end{tabular}
\end{table}
...
\hline Gr＿total＿mess & ages＿sent＿count & & & Yes & Both & No \\
\hline Gr＿transactions & s＿consistency & after＿sync＿co & unt & Yes & Both & No \\
\hline Gr＿transactions & s＿consistency & after＿sync＿tim & e＿sum & Yes & Both & No \\
\hline Gr＿transactions & & s＿consistency＿after＿terminat & ation＿count & Yes & Both & No \\
\hline Gr＿transactions & s＿consistency＿after＿termina & & tion＿time＿sum & Yes & Both & No \\
\hline Gr＿transactions & s＿consistency＿before＿begin & & count & Yes & Both & No \\
\hline Gr＿transactions & s＿consistency＿before＿begin & & time＿sum & Yes & Both & No \\
\hline group＿replicatii & ぬesadvertise＿reesvery＿endp & & díats & & Global & Yes \\
\hline group＿replicatii & øesallow＿localY & Yesver＿versio & Yesin & & Global & Yes \\
\hline group＿replicatii & & ぬesauto＿incremest＿incremen & Yes & & Global & Yes \\
\hline group＿replicatii & iresautorejoin & Veess & Yes & & Global & Yes \\
\hline group＿replicat & iresbootstrap＿ & gresip & Yes & & Global & Yes \\
\hline group＿replicat & iresclone＿thres⿳ & sheesd & Yes & & Global & Yes \\
\hline group＿replicati & firescommunicał & ariest＿debug＿o & ptress & & Global & Yes \\
\hline group＿replicat & irescommunica & ariest＿max＿me & sisase＿size & & Global & Yes \\
\hline group＿replicati & ![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3725.jpg?height=42\&width=255\&top_left_y=1174\&top_left_x=583) & ation＿stack & Yes & & Global & Yes \\
\hline group＿replicat & irescomponent\＄ & tyestop＿timeou & tres & & Global & Yes \\
\hline group＿replicat & irescompressio & oreshreshold & Yes & & Global & Yes \\
\hline group＿replicat & iresconsistency） & & Yes & & Both & Yes \\
\hline group＿replicat & iresenforce＿upb & phase＿everywh & \＆es checks & & Global & Yes \\
\hline group＿replicat & iresexit＿state＿ & yetson & Yes & & Global & Yes \\
\hline group＿replicat & iresflow＿controł & Orespplier＿thre & SKendd & & Global & Yes \\
\hline group＿replicat & iresflow＿contro & OYesertifier＿thre & Slesld & & Global & Yes \\
\hline group＿replicat & iresflow＿controł & Yessold＿percen & tYes & & Global & Yes \\
\hline group＿replicati & iresflow＿controł & Oressiax＿quota & Yes & & Global & Yes \\
\hline group＿replicat & iresflow＿contro & Oresblember＿qu & dłaspercent & & Global & Yes \\
\hline group＿replicati & iresflow＿contro & oressin＿quota & Yes & & Global & Yes \\
\hline group＿replicat & iresflow＿controł & OYessin＿recove & Yesquota & & Global & Yes \\
\hline group＿replicat & iresflow＿contro & oressiode & Yes & & Global & Yes \\
\hline group＿replicat & iresflow＿controł & Yeqseriod & Yes & & Global & Yes \\
\hline group＿replicat & iresflow＿controł & Yeqelease＿per & deet & & Global & Yes \\
\hline group＿replicat & iresforce＿memb & bess & Yes & & Global & Yes \\
\hline group＿replicat & iresgroup＿namk & iées & Yes & & Global & Yes \\
\hline group＿replicat & iresgroup＿seed & U\＆es & Yes & & Global & Yes \\
\hline group＿replicati & iresgtid＿assignr & Yræsnt＿block＿S & iYes & & Global & Yes \\
\hline group＿replicati & iresip＿allowlist & Yes & Yes & & Global & Yes \\
\hline group＿replicat & iqueslocal＿addrets & Asss & Yes & & Global & Yes \\
\hline group＿replicat & iresmember＿ex & ＊timeout & Yes & & Global & Yes \\
\hline group＿replicat． & iresmember＿we & & Yes & & Global & Yes \\
\hline group＿replicat & iresmessage＿ & cke\＄e＿size & Yes & & Global & Yes \\
\hline group＿replicati & irespaxos＿sing & Yeseader & Yes & & Global & Yes \\
\hline
\end{tabular}

\begin{tabular}{|l|l|l|l|l|l|}
\hline Name & Option File & System Var & Status Var & Var Scope & Dynamic \\
\hline group_replicatiørespoll_spin_ & docass & Yes & & Global & Yes \\
\hline group_replicatiørespreemptive & Yegarbage_coll & Hacton & & Global & Yes \\
\hline group_replicatiørespreemptive & Ygarbage_coll & Udcisn_rows_th & reshold & Global & Yes \\
\hline group_replicatiøresrecovery_g & geespublic_key & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & públic_key_pat & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & r\&eesnnect_inte & Wads & & Global & Yes \\
\hline group_replicatiøresrecovery_r & retes_count & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$8lesa & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$lesapath & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$lesert & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$lesipher & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$lesrl & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$lestripath & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$lesey & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_ & \$\$legerify_serve & eYesert & & Global & Yes \\
\hline group_replicatiøresrecovery_ & I\$esiphersuites & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_t & I|\$esersion & Yes & & Global & Yes \\
\hline group_replicatiøresrecovery_u & uresssl & Yes & & Global & Yes \\
\hline group_replicatiøressingle_prim & Mæゆ_mode & Yes & & Global & Yes \\
\hline group_replicatiøresssl_mode & Yes & Yes & & Global & Yes \\
\hline group_replicatiøresstart_on_b & docts & Yes & & Global & Yes \\
\hline group_replicatiørestransaction & Yeise_limit & Yes & & Global & Yes \\
\hline group_replicatiøresunreachab & Kesnajority_tin & とest & & Global & Yes \\
\hline group_replicatiøresview_chan & gyesuuid & Yes & & Global & Yes \\
\hline
\end{tabular}

\subsection*{20.9.1 Group Replication System Variables}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3728.jpg?height=102&width=106&top_left_y=1546&top_left_x=301)
...
\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-advertise-recovery-endpoints=value & \multirow{4}{*}{endpoints} \\
\hline System Variable & group_replication_advertise_recovery_ & \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline
\end{tabular}
...
group_replication_advertise_recovery_endpoints= "127.0.0.1:3306,127.0.0.1:4567,[::1]:3306,localhost:3
...
\hline System Variable & group_replication_allow_local_lower_version_jo \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
...
group_replication_allow_local_lower_version_join allows the current server to join the group even if it is running a lower MySQL Server version than the group. With the default setting 0FF, servers are not permitted to join a replication group if they are running a lower version than the existing group members. This standard policy ensures that all members of a group are able to exchange messages and apply transactions.
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3730.jpg?height=125&width=113&top_left_y=1427&top_left_x=333)
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-auto-incrementincrement=\# \\
\hline System Variable & group_replication_auto_increment_increment \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
\hline Default Value & 7 \\
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3731.jpg?height=115&width=103&top_left_y=897&top_left_x=402)
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{- group_replication_autorejoin_tries}
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-autorejointries=\# \\
\hline System Variable & group_replication_autorejoin_tries \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
group_replication_autorejoin_tries specifies the number of tries that a member makes to automatically rejoin the group if it is expelled, or if it is unable to contact a majority of the group before the group_replication_unreachable_majority_timeout setting is reached. When the member's expulsion or unreachable majority timeout is reached, it makes an attempt to rejoin (using the current plugin option values), then continues to make further autorejoin attempts up to the specified number of tries. After an unsuccessful auto-rejoin attempt, the member waits 5 minutes before the next try. If the specified number of tries is exhausted without the member rejoining or being stopped, the member proceeds to the action specified by the group_replication_exit_state_action system variable.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-bootstrapgroup[=\{OFF|ON\}] \\
\hline System Variable & group_replication_bootstrap_group \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
\hline
\end{tabular}
group_replication_bootstrap_group configures this server to bootstrap the group. This system variable must only be set on one server, and only when starting the group for the first time or restarting the entire group. After the group has been bootstrapped, set this option to 0FF. It should be set to OFF both dynamically and in the configuration files. Starting two servers or restarting one server with this option set while the group is running may lead to an artificial split brain situation, where two independent groups with the same name are bootstrapped.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-clonethreshold=\# \\
\hline System Variable & group_replication_clone_threshold \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
\hline Default Value & 9223372036854775807 \\
\hline Minimum Value & 1 \\
\hline Maximum Value & 9223372036854775807 \\
\hline Unit & transactions \\
\hline
\end{tabular}
...
\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-communication-debug-options=value & \multirow{8}{*}{optio} \\
\hline System Variable & group_replication_communication_debug & \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & String & \\
\hline Default Value & GCS_DEBUG_NONE & \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-communication-max-message-size=\# \\
\hline System Variable & group_replication_communication_max_message_si \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
\hline Default Value & 10485760 \\
\hline Minimum Value & 0 \\
...
A maximum message size of 10485760 bytes ( 10 MiB ) is set by default, which means that fragmentation is used by default. The greatest permitted value is the same as the maximum value of the replica_max_allowed_packet system variable, which is 1073741824 bytes
( 1 GB ). group_replication_communication_max_message_size must be less than replica_max_allowed_packet, because the applier thread cannot handle message fragments larger than the maximum permitted packet size. To switch off fragmentation, set group_replication_communication_max_message_size to 0 .
...
\begin{tabular}{|l|l|}
\hline System Variable & group_replication_communication_stack \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
\hline Default Value & XCOM \\
\hline Valid Values & \begin{tabular}{l}
XCOM \\
MYSQL
\end{tabular} \\
...
group_replication_communication_stack is effectively a group-wide configuration setting, and the setting must be the same on all group members. However, this is not policed by Group Replication's own checks for group-wide configuration settings. A member with a different value from
the rest of the group cannot communicate with the other members at all, because the communication protocols are incompatible, so it cannot exchange information about its configuration settings.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-components-stoptimeout=\# \\
\hline System Variable & group_replication_components_stop_timeout \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-compressionthreshold=\# \\
\hline System Variable & group_replication_compression_threshold \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replicationconsistency=value \\
\hline System Variable & group_replication_consistency \\
\hline Scope & Global, Session \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Enumeration \\
\hline Default Value & BEFORE_ON_PRIMARY_FAILOVER \\
\hline Valid Values & \begin{tabular}{l}
EVENTUAL \\
...
group_replication_consistency determines the transaction consistency guarantee which a group provides; this can done globally, or per transaction. group_replication_consistency also determines the fencing mechanism used by newly elected primaries in single primary groups. The effect of the variable must be considered both for read-only and for read/write transactions. The
following list shows the possible values of this variable, in order of increasing transaction consistency guarantee:
- EVENTUAL
...
\begin{tabular}{|l|l|}
\hline System Variable & group_replication_enforce_update_everywhere \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-exit-stateaction=value \\
\hline System Variable & group_replication_exit_state_action \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Enumeration \\
\hline Default Value & OFFLINE_MODE \\
\hline Valid Values & \begin{tabular}{l}
ABORT_SERVER \\
OFFLINE_MODE \\
READ_ONLY
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-applier-threshold=\# \\
\hline System Variable & group_replication_flow_control_applier_thre \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-certifier-threshold=\# \\
\hline System Variable & group_replication_flow_control_certiflier_th \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-hold-percent=\# \\
\hline System Variable & group_replication_flow_control_hold_percent \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-max-quota=\# \\
\hline System Variable & group_replication_flow_control_max_quota \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-flow-control-member-quota-percent=\# & \\
\hline System Variable & group_replication_flow_control_member_quota_pe & \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & Integer & \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-min-quota=\# \\
\hline System Variable & group_replication_flow_control_min_quota \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-min-recovery-quota=\# \\
\hline System Variable & group_replication_flow_control_min_recovery \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-controlmode=value \\
\hline System Variable & group_replication_flow_control_mode \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Enumeration \\
\hline Default Value & QUOTA \\
\hline Valid Values & \begin{tabular}{l}
DISABLED \\
QUOTA
\end{tabular} \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-controlperiod=\# \\
\hline System Variable & group_replication_flow_control_period \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-flow-control-release-percent=\# \\
\hline System Variable & group_replication_flow_control_release_percent \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-forcemembers=value \\
\hline System Variable & group_replication_force_members \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-group-name=value \\
\hline System Variable & group_replication_group_name \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3746.jpg?height=104&width=103&top_left_y=1050&top_left_x=338)
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-groupseeds=value \\
\hline System Variable & group_replication_group_seeds \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
When a server is joining a replication group, it attempts to connect to the first seed member listed in its group_replication_group_seeds system variable. If the connection is refused, the joining member tries to connect to each of the other seed members in the list in order. If the joining member
connects to a seed member but does not get added to the replication group as a result (for example, because the seed member does not have the joining member's address in its allowlist and closes the connection), the joining member continues to try the remaining seed members in the list in order.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-gtid-assignment-block-size=\# \\
\hline System Variable & group_replication_gtid_assignment_block_siz \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
\hline Default Value & 1000000 \\
\hline Minimum Value & 1 \\
\hline Maximum Value (64-bit platforms) & 9223372036854775807 \\
...
allowlist=value
\end{tabular} \\
\hline System Variable & group_replication_ip_allowlist \\
\hline Scope & Global \\
\hline
\end{tabular}
...
alternative address for the joining member for the protocol offered by the seed member (or a host name that resolves to an address for that protocol). For more information, see Section 20.6.4, "Group Replication IP Address Permissions".
...
For host names, name resolution takes place only when a connection request is made by another server. A host name that cannot be resolved is not considered for allowlist validation, and a warning message is written to the error log. Forward-confirmed reverse DNS (FCrDNS) verification is carried out for resolved host names.
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3749.jpg?height=110&width=103&top_left_y=884&top_left_x=402)
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-localaddress=value \\
\hline System Variable & group_replication_local_address \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3749.jpg?height=106&width=111&top_left_y=2339&top_left_x=397)
...
The address or host name that you specify in group_replication_local_address is used by Group Replication as the unique identifier for a group member within the replication group. You can use the same port for all members of a replication group as long as the host names or IP addresses are all different, and you can use the same host name or IP address for all members as long as the ports are all different. The recommended port for group_replication_local_address
is 33061 . Note that the value you specify for this variable is not validated until the START GROUP_REPLICATION statement is issued and the Group Communication System (GCS) is available.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-member-expeltimeout=\# \\
\hline System Variable & group_replication_member_expel_timeout \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
group_replication_member_expel_timeout defaults to 5, meaning that a suspected member is liable for expulsion 5 seconds after the 5 -second detection period.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-member-weight=\# \\
\hline System Variable & group_replication_member_weight \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-message-cachesize=\# \\
\hline System Variable & group_replication_message_cache_size \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
\hline Default Value & 1073741824 ( 1 GB ) \\
\hline Minimum Value & 134217728 ( 128 MB ) \\
\hline Maximum Value (64-bit platforms) & 18446744073709551615 (16 EiB) \\
\hline Maximum Value (32-bit platforms) & 315360004294967295 ( 4 GB ) \\
\hline Unit & bytes \\
\hline
\end{tabular}
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-paxos-singleleader[=\{OFF|ON\}] \\
\hline System Variable & group_replication_paxos_single_leader \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
This system variable is a group-wide configuration setting, and a full reboot of the replication group is required for a change to take effect.
group_replication_paxos_single_leader enables the group communication engine to operate with a single consensus leader when the group is in single-primary mode. With the default setting 0FF, this behavior is disabled, and every member of the group is used as a leader, which is the behavior in releases before this system variable was available. When this variable is set to ON, the group communication engine can use a single leader to drive consensus. Operating with a single consensus leader improves performance and resilience in single-primary mode, particularly when some of the group's secondary members are currently unreachable. For more information, see Section 20.7.3, "Single Consensus Leader".
...
The WRITE_CONSENSUS_SINGLE_LEADER_CAPABLE column of the Performance Schema table replication_group_communication_information shows whether the group supports the use of a single leader, even if group_replication_paxos_single_leader is
currently set to OFF on the queried member. The column value is 1 if the group was started with group_replication_paxos_single_leader set to ON, and its communication protocol version is MySQL 8.0.27 or later.
- group_replication_poll_spin_loops

\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-poll-spinloops=\# \\
\hline System Variable & group_replication_poll_spin_loops \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-preemptive-garbage-collection[=ON|OFF] \\
\hline System Variable & group_replication_preemptive_garbage_collectio \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-preemptive-garbage-collection-rows-threshold=\# & \multirow{9}{*}{collec} \\
\hline System Variable & group_replication_preemptive_garbage & \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & Integer & \\
\hline Default Value & 100000 & \\
\hline Minimum Value & 10000 & \\
\hline Maximum Value & 100000000 & \\
\hline
\end{tabular}
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-compression-algorithms=value \\
\hline System Variable & group_replication_recovery_compression_algo \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Set \\
...
\hline
\end{tabular}
group_replication_recovery_compression_algorithms specifies the compression algorithms permitted for Group Replication distributed recovery connections for state
transfer from a donor's binary log. The available algorithms are the same as for the protocol_compression_algorithms system variable. For more information, see Section 6.2.8, "Connection Compression Control".
...
\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-recovery-get-public-key[=\{OFF|ON\}] & \\
\hline System Variable & group_replication_recovery_get_public_key & \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & Boolean & \\
...
The value of this system variable can be changed while Group Replication is running, but the change takes effect only after you stop and restart Group Replication on the group member.
group_replication_recovery_public_key_path specifies the path name to a file containing a replica-side copy of the public key required by the source for RSA key pair-based password exchange. The file must be in PEM format. If group_replication_recovery_public_key_path is set to a valid public key file, it takes precedence over group_replication_recovery_get_public_key. This variable applies
if you are not using SSL for distributed recovery over the group_replication_recovery channel (so group_replication_recovery_use_ssl is set to 0FF), and the replication user account for Group Replication authenticates with the caching_sha2_password plugin (the default) or the sha256_password plugin (deprecated). (For sha256_password, setting group_replication_recovery_public_key_path applies only if MySQL was built using OpenSSL.) For more details, see Replication User With The Caching SHA-2 Authentication Plugin.
- group_replication_recovery_reconnect_interval

\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-recovery-reconnect-interval=\# & \\
\hline System Variable & group_replication_recovery_reconnect & interv \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & Integer & \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-retrycount=\# \\
\hline System Variable & group_replication_recovery_retry_count \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
ca=value
\end{tabular} \\
\hline System Variable & group_replication_recovery_ssl_ca \\
\hline
\end{tabular}
...
If this server has been set up to support cloning (see Section 20.5.4.2, "Cloning for Distributed Recovery"), and you have set group_replication_recovery_use_ssl to 0N, Group Replication automatically configures the setting for the clone SSL option clone_ssl_ca to match your setting for group_replication_recovery_ssl_ca.
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-sslcapath=value \\
\hline System Variable & group_replication_recovery_ssl_capath \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-sslcert=value \\
\hline System Variable & group_replication_recovery_ssl_cert \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-sslcipher=value \\
\hline System Variable & group_replication_recovery_ssl_cipher \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-sslcrl=value \\
\hline System Variable & group_replication_recovery_ssl_crl \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & File name \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-sslcrlpath=value \\
\hline System Variable & group_replication_recovery_ssl_crlpath \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-sslkey=value \\
\hline System Variable & group_replication_recovery_ssl_key \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
If this server has been set up to support cloning (see Section 20.5.4.2, "Cloning for Distributed Recovery"), and you have set group_replication_recovery_use_ssl to 0N, Group Replication automatically configures the setting for the clone SSL option clone_ssl_key to match your setting for group_replication_recovery_ssl_key.

When the MySQL communication stack is in use for the group (group_replication_communication_stack = MYSQL), this setting is used for the TLS/SSL configuration for group communication connections, as well as for distributed recovery connections.
- group_replication_recovery_ssl_verify_server_cert

\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-recovery-ssl-verify-server-cert[=\{OFF|ON\}] & \\
\hline System Variable & group_replication_recovery_ssl_verify_serve & \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & Boolean & \\
...
\begin{tabular}{|l|l|l|}
\hline Command-Line Format & --group-replication-recovery-tlsciphersuites=value & \\
\hline System Variable & group_replication_recovery_tls_cipher & suites \\
\hline Scope & Global & \\
\hline Dynamic & Yes & \\
\hline SET_VAR Hint Applies & No & \\
\hline Type & String & \\
...
\begin{tabular}{|l|l|}
\hline System Variable & group_replication_recovery_tls_version \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3762.jpg?height=118&width=103&top_left_y=1297&top_left_x=338)
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-usessl[=\{OFF|ON\}] \\
\hline System Variable & group_replication_recovery_use_ssl \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
If this server has been set up to support cloning (see Section 20.5.4.2, "Cloning for Distributed Recovery"), and you set this option to 0N, Group Replication uses SSL for remote cloning operations as well as for state transfer from a donor's binary log. If you set this option to 0FF, Group Replication does not use SSL for remote cloning operations.
- group_replication_recovery_zstd_compression_level

\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-recovery-zstd-compression-level=\# \\
\hline System Variable & group_replication_recovery_zstd_compr \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-single-primarymode[=\{OFF|ON\}] \\
\hline System Variable & group_replication_single_primary_mode \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-ssl-mode=value \\
\hline System Variable & group_replication_ssl_mode \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Enumeration \\
\hline Default Value & DISABLED \\
\hline Valid Values & \begin{tabular}{l}
DISABLED \\
REQUIRED \\
VERIFY_CA \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-start-onboot[=\{OFF|ON\}] \\
\hline System Variable & group_replication_start_on_boot \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Boolean \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-tls-source=value \\
\hline System Variable & group_replication_tls_source \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Enumeration \\
\hline Default Value & mysql_main \\
\hline Valid Values & \begin{tabular}{l}
mysql_main \\
mysql_admin
\end{tabular} \\
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-transaction-size-limit=\# \\
\hline System Variable & group_replication_transaction_size_limit \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
\hline Default Value & 150000000 \\
\hline Minimum Value & 0 \\
\hline Maximum Value & 2147483647 \\
\hline Unit & bytes \\
\hline
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-unreachable-majority-timeout=\# \\
\hline System Variable & group_replication_unreachable_majority_timeout \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & Integer \\
...
group_replication_unreachable_majority_timeout specifies a number of seconds for which members that suffer a network partition and cannot connect to the majority wait before leaving the group. In a group of 5 servers ( $\mathrm{S} 1, \mathrm{~S} 2, \mathrm{~S} 3, \mathrm{~S} 4, \mathrm{~S} 5$ ), if there is a disconnection between ( $\mathrm{S} 1, \mathrm{~S} 2$ ) and ( $\mathrm{S} 3, \mathrm{~S} 4, \mathrm{~S} 5$ ) there is a network partition. The first group ( $\mathrm{S} 1, \mathrm{~S} 2$ ) is now in a minority because it cannot contact more than half of the group. While the majority group ( $\mathrm{S} 3, \mathrm{~S} 4, \mathrm{~S} 5$ ) remains running, the minority group waits for the specified time for a network reconnection. For a detailed description of this scenario, see Section 20.7.8, "Handling a Network Partition and Loss of Quorum".
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3767.jpg?height=127&width=108&top_left_y=1260&top_left_x=397)
...
\begin{tabular}{|l|l|}
\hline Command-Line Format & --group-replication-view-changeuuid=value \\
\hline Deprecated & Yes \\
\hline System Variable & group_replication_view_change_uuid \\
\hline Scope & Global \\
\hline Dynamic & Yes \\
\hline SET_VAR Hint Applies & No \\
\hline Type & String \\
...
\subsection*{20.9.2 Group Replication Status Variables}
...
\subsection*{20.10 Frequently Asked Questions}

This section provides answers to frequently asked questions.

\section*{What is the maximum number of MySQL servers in a group?}
...
\section*{How are servers in a group connected?}

Servers in a group connect to the other servers in the group by opening a peer-topeer TCP connection. These connections are only used for internal communication and message passing between servers in the group. This address is configured by the group_replication_local_address variable.

\section*{What is the group_replication_bootstrap_group option used for?}
...
\section*{How do I set credentials for the distributed recovery process?}

You can set the user credentials permanently as the credentials for the group_replication_recovery channel, using a CHANGE REPLICATION SOURCE TO statement.

You can specify them in the START GROUP_REPLICATION statement each time Group Replication is started.
...
\section*{Can I scale-out my write-load using Group Replication?}
...
\section*{Does Group Replication require more network bandwidth and CPU, when compared to simple replication and under the same workload?}
...
\section*{Can I deploy Group Replication across wide-area networks?}
...
\section*{Do members automatically rejoin a group in case of temporary connectivity problems?}
...
\section*{When is a member excluded from a group?}
...
\section*{What happens when one node is significantly lagging behind?}
...
\section*{Upon suspicion of a problem in the group, is there a special member responsible for triggering a reconfiguration?}
...
\section*{Can I use Group Replication for sharding?}
...
\section*{How do I use Group Replication with SELinux?}
...
\section*{How do I use Group Replication with iptables?}
...
\section*{How do I recover the relay log for a replication channel used by a group member?}
...
\section*{Why does Group Replication use two bind addresses?}
...
\section*{How does Group Replication use network addresses and hostnames?}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-3773.jpg?height=106&width=106&top_left_y=2238&top_left_x=365)
...
\section*{Why did the auto increment setting on the server change?}

When Group Replication is started on a server, the value of auto_increment_increment is changed to the value of group_replication_auto_increment_increment, which defaults to 7,
and the value of auto_increment_offset is changed to the server ID. The changes are reverted when Group Replication is stopped. These settings avoid the selection of duplicate auto-increment values for writes on group members, which causes rollback of transactions. The default auto increment value of 7 for Group Replication represents a balance between the number of usable values and the permitted maximum size of a replication group ( 9 members).
...
\section*{How do I find the primary?}
