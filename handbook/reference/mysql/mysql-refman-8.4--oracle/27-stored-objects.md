\section*{Chapter 27 Stored Objects}
Table of Contents
...
\subsection*{27.1 Defining Stored Programs}
...
\subsection*{27.2 Using Stored Routines}
...
\subsection*{27.2.1 Stored Routine Syntax}
...
- When the routine is invoked, an implicit USE $d b \_n$ ame is performed (and undone when the routine terminates). USE statements within stored routines are not permitted.
...
\subsection*{27.2.2 Stored Routines and MySQL Privileges}
...
\subsection*{27.2.3 Stored Routine Metadata}
...
\subsection*{27.2.4 Stored Procedures, Functions, Triggers, and LAST_INSERT_ID()}
...
\subsection*{27.3 Using Triggers}
...
\subsection*{27.3.1 Trigger Syntax and Examples}
...
\subsection*{27.3.2 Trigger Metadata}

To obtain metadata about triggers:
- Query the TRIGGERS table of the INFORMATION_SCHEMA database. See Section 28.3.44, "The INFORMATION_SCHEMA TRIGGERS Table".
...
\subsection*{27.4 Using the Event Scheduler}
...
\subsection*{27.4.1 Event Scheduler Overview}
...
\subsection*{27.4.2 Event Scheduler Configuration}
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-4625.jpg?height=106&width=108&top_left_y=1053&top_left_x=365)
...
![](https://cdn.mathpix.com/cropped/3d8cf8cc-cc66-4dae-b696-9a5a902951f8-4625.jpg?height=122&width=99&top_left_y=1946&top_left_x=370)
...
\subsection*{27.4.3 Event Syntax}
...
\subsection*{27.4.4 Event Metadata}
...
\section*{Event Scheduler Time Representation}
...
\begin{tabular}{|l|l|l|}
\hline Value & EVENTS Table & SHOW EVENTS \\
\hline Execute at & ETZ & ETZ \\
\hline Starts & ETZ & ETZ \\
\hline Ends & ETZ & ETZ \\
\hline Last executed & ETZ & n/a \\
\hline Created & STZ & n/a \\
\hline Last altered & STZ & n/a \\
\hline
\end{tabular}

\subsection*{27.4.5 Event Scheduler Status}
...
\subsection*{27.4.6 The Event Scheduler and MySQL Privileges}
...
\subsection*{27.5.2 View Processing Algorithms}
...
\subsection*{27.5.3 Updatable and Insertable Views}
...
\subsection*{27.5.4 The View WITH CHECK OPTION Clause}
...
Here the v 2 and v 3 views are defined in terms of another view, v 1 .
...
\subsection*{27.5.5 View Metadata}

To obtain metadata about views:
- Query the VIEWS table of the INFORMATION_SCHEMA database. See Section 28.3.47, "The INFORMATION_SCHEMA VIEWS Table".
- Use the SHOW CREATE VIEW statement. See Section 15.7.7.14, "SHOW CREATE VIEW Statement".

\subsection*{27.6 Stored Object Access Control}
...
\section*{The SQL SECURITY Characteristic}
...
\section*{Risk-Minimization Guidelines}
...
\subsection*{27.7 Stored Program Binary Logging}
...
- To create or alter a stored function, you must have the SET_ANY_DEFINER privilege, in addition to the CREATE ROUTINE or ALTER ROUTINE privilege that is normally required. (Depending on the DEFINER value in the function definition, SET_ANY_DEFINER might be required regardless of whether binary logging is enabled. See Section 15.1.17, "CREATE PROCEDURE and CREATE FUNCTION Statements".)
...
When the SELECT statement executes, the function f 1() is invoked three times. Two of those invocations insert a row, and MySQL logs a SELECT statement for each of them. That is, MySQL writes the following statements to the binary log:
```
SELECT f1(1);
SELECT f1(2);
```
...
\subsection*{27.8 Restrictions on Stored Programs}
...
\section*{SQL Statements Not Permitted in Stored Routines}
...
\section*{Restrictions for Stored Functions}
...
\section*{Restrictions for Triggers}
...
\section*{Name Conflicts within Stored Routines}
...
\section*{Unsupported Syntax from the SQL:2003 Standard}
...
\section*{Stored Routine Concurrency Considerations}
...
\section*{Event Scheduler Restrictions}
...
\section*{Stored routines and triggers in NDB Cluster}
...
\subsection*{27.9 Restrictions on Views}
