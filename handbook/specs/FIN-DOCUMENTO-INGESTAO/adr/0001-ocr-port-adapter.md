# ADR-0001: OCR como Port/Adapter Pattern
...
1. Definir `OcrPort` (interface pura no domain) com método `extract(pdfUrl: string): Promise<OcrResult>`.
2. Implementar adapters concretos fora do domínio:
   - `OcrMockAdapter` (fase 1 — desenvolvimento e testes)
   - `TesseractOcrAdapter` (fase 2 — opção open source)
   - `AwsTextractAdapter` (fase 3 — opção SaaS, se necessário)
...
- **Testabilidade**: `OcrMockAdapter` permite testes unitários determinísticos.
...
Tech Lead + Especialista de Domínio
