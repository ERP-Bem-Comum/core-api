...
| [0066](./0066-semver-product-version-and-generated-changelog.md) | **A versão do produto é SemVer com tag anotada `vX.Y.Z`, e o `CHANGELOG.md` é gerado do git** — eixo **distinto** do [0033](./0033-api-versioning-v1-legacy-mirror.md), que versiona a API por recurso. Levantado em 25/08: o repositório nunca marcou versão — **zero** tags de versão (as 17 são `archive/*`/`backup/*`/`legacy/*`), **zero** releases, `package.json` em `0.1.0`, sem CHANGELOG, com `main` (que dispara o deploy) parada em `9acf6db1`, 23/07, a **572 commits** (**192** por `--first-parent`) da `dev`. Rejeita `1.0.0-RELEASE` por **incorreção**: pela regra 9 do SemVer tudo após o hífen é *pré-lançamento*, logo `1.0.0-RELEASE` **<** `1.0.0` — a forma é convenção Maven/Spring (`1.0.0.RELEASE`, com ponto). Rejeita **LTS**, que é política de suporte a linhas paralelas e não número de versão, num produto de um ambiente e um cliente. Escada `-alpha.N` → `-beta.N` → `-rc.N` → GA; `1.0.0` **é promessa**: depois dele quebrar contrato exige major, e foi sob a licença do `0.y.z` (regra 4) que o [#826](https://github.com/ERP-Bem-Comum/core-api/issues/826) entrou. CHANGELOG **derivado** por `pnpm release:notes`, que **erra para mostrar** — `chore` de escopo desconhecido ou ausente sai em seção visível, porque `chore(financial)` do [#855](https://github.com/ERP-Bem-Comum/core-api/pull/855) ligou download de dado bancário em produção — e **nomeia** o que não classificou. **A capacidade entra na `dev`; a declaração não** — gerador, suíte e política são infraestrutura, enquanto `package.json#version` e o `CHANGELOG.md` são afirmações sobre o artefato e só nascem no ato da promoção: mergear `1.0.0-rc.1` na `dev` faria todo commit posterior nascer dentro de uma rc que não o contém, e `0.1.0` na `dev` é **correto** enquanto nada foi publicado. ⚠️ **Não autoriza a promoção**: quem a libera é o CA0 da [#873](https://github.com/ERP-Bem-Comum/core-api/issues/873), o RBAC hoje fixado em `bypass` (`src/server.ts:161`). **A CERIMÔNIA de validação roda EM PRODUÇÃO** (decisão da equipe de negócio), então a ordem é o inverso do intuitivo: resolver o CA0 → promover a `-rc.N` para `main` (o deploy) → cerimônia **ponta a ponta até a VAN em produção** → declarar o GA sobre a **mesma árvore**. A promoção é pré-condição da cerimônia, não o contrário — e é por isso que `-rc.N` existe: a candidata **vai a produção sendo candidata**, e reprovando o recuo se chama `-rc.2` sem queimar o `1.0.0`. ⚠️ A ordem torna o CA0 **pré-requisito de deploy**, não de release, e faz a cerimônia esbarrar no CA2: gravar no `saida/` **é** enfileirar pagamento (ADR-0060) | Accepted | 2026-08-25 |
