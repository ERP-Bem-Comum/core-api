...
* **Gestor Financeiro**: Responsável pela aprovação de pagamentos e autorização de remessas bancárias.
...
* **Sistema (Automatismos)**: O motor de retenções e o processador de arquivos CNAB (Remessa/Retorno via VAN).
...
3. **Aprovação**: O Gestor sela o documento. Uma vez aprovado, os valores tornam-se imutáveis (travados para edição direta).
...
6. **Conciliação**: O extrato bancário é vinculado ao título, alterando o status para `LIQUIDADO`.
...
* ✅ Geração de arquivos CNAB (Remessa/Retorno).
* ✅ Gestão de Status (Aberto, Aprovado, Transmitido, Recusado, Pago, Liquidado).
