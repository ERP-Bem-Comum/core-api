...
### Pendências de backend agora registradas → [CTR-CONTRACT-METADATA-E-ADITIVOS](./CTR-CONTRACT-METADATA-E-ADITIVOS.md)

As três pendências que estavam "sem ticket" foram consolidadas no card acima:

- **Persistência de metadados do contrato**: programa, categoria, centro de custo, plano orçamentário,
  classificação (CT/OS) — não há colunas; hoje exibidos como "—" no detalhe. (`programa` depende da feature 008.)
