// Config do worker de outbox lida do AMBIENTE (12-factor) — sem dependência de CLI.
// CLI-RETIRE-EMBEDDED: substitui o parsing de flags do antigo `cli/commands/run-outbox-worker.ts`.
...
export type WorkerRuntimeConfig = Readonly<{
  /** Connection string do MySQL (writer) — o worker lê/atualiza `ctr_outbox`. */
...
/**
 * Lê a config do worker do ambiente. `CONTRACTS_DATABASE_URL` é obrigatório (mesma env do
