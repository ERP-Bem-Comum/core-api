...
//
// Guarda as linhas por documento e devolve apenas as pedidas, na ordem da consulta. Não inventa
...
export const createInMemoryRemittancePreviewReader = (
  ...
  const rows = new Map<string, RemittancePreviewRow>(seedRows.map((r) => [r.documentId, r]));
  ...
  return {
    ...
    loadPreviewRows: async (
      ...
      const found = documentIds.flatMap((id) => {
