...
// `generate-remittance` produz com `toISOString()`, e o que o agente publica em `executadoEm`.
...
//
// O defeito só aparecia contra MySQL real — as suítes exercitam o repo in-memory, e o caminho
// `POST /financial/remittances` sempre morria em `remittance-persist-failed` (503) fora dele.
//
// A conversão fica AQUI porque o formato é do MySQL, não do domínio: quem conhece o dialeto é o
// adapter (ADR-0006). `datetime` não guarda fuso, então a ida e a volta são ambas em UTC — o
// instante gravado é o mesmo que se lê, independente do `time_zone` da sessão.
const pad = (value: number, width = 2): string => String(value).padStart(width, '0');

const toMysqlDateTime = (iso: string): string => {
  const at = new Date(iso);
  // String que não é instante reconhecível segue crua: falhar no banco é melhor que gravar
  // silenciosamente um valor inventado numa coluna que decide quando um pagamento saiu.
...
// `2026-08-18 15:01:24.615` e o front receberia um instante que `Date` não parseia em todo browser.
const toIsoDateTime = (stored: string): string =>
