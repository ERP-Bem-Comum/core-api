/**
 * Porta pública de migração do módulo financial (CORE-MIGRATE-JOB / ADR-0006).
 *
...
export type ApplyFinancialMigrationsError = FinancialMysqlDriverError;

export const applyMigrations = async (
  connectionString: string,
): Promise<Result<true, ApplyFinancialMigrationsError>> => {
  const handleR = await openMysqlFinancial({ connectionString, applyMigrations: true });
