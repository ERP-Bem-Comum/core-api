...
// markFailed / moveToDeadLetter) POR CONSUMIDOR (#800, #824) — esta tabela e consumida desde ja
// pelo email-dispatch. O progresso vive em `createInMemoryProgressStore`, o espelho em memoria de
// `eventos_processados`, nunca na propria row do outbox. DLQ SEM tabela dedicada nesta fatia:
// `moveToDeadLetter` marca `deadLetteredAt` no progresso DESTE consumidor (sai do pending pool
// DELE, preserva a row para os demais e para auditoria). Ver email-outbox-repository.drizzle.ts.
//
...
export const InMemoryParEmailOutbox = (): {
  ...
  // outbox segue intacta para os demais consumidores e para auditoria.
