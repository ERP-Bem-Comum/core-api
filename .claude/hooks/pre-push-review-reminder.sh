...
if ! printf '%s' "$command" | grep -qE '(^|[;&|]|&&|\|\||^[[:space:]]*)[[:space:]]*git[[:space:]]+push\b'; then
  exit 0
fi
...
if git diff --name-only "$base"...HEAD 2>/dev/null |
  grep -qE '^(src/modules/auth/|src/shared/(http|security)/|\.github/workflows/|compose|Dockerfile|src/modules/financial/adapters/cnab/)'; then
