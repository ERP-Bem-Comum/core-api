...
### `chown user:group a` *(requer root em geral)*
...
Para *script* que só precisa testar:
...
const hash = createHash('sha256').update(await readFile('arquivo')).digest('hex');
```
