...
await Promise.all([
  pipeline(c1.stdout!, c2.stdin!),
  pipeline(c2.stdout!, c3.stdin!),
]);
```
...
```ts
const a = await run('cmd1', []); if (!a.ok) return falhar(a.error);
const b = await run('cmd2', []); if (!b.ok) return falhar(b.error);
```
...
process.on('exit', () => { /* ações síncronas */ });
