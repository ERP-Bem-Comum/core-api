...
| 3b | `AUTH-HTTP-ROUTES-SESSION` (H1b) | S | `POST /api/v2/auth/{refresh,logout}` → reusa composition root (use cases já instanciados); rotação + revoke idempotente (DD-SESSION) | H1a |
