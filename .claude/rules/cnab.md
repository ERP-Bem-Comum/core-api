...
- **Existem 13 `.REM` que o banco aceitou, e a receita deles está medida** — [inquiry-0033](../../handbook/inquiries/0033-cnab-multipag-bisseccao-validador.md), 18 submissões ao Validador Universal em 25/08/2026, uma variável por arquivo. O que ela prova, e o layout não diz:

  | Posição | TED (`41`) | Crédito em conta (`01`) |
  | :--- | :--- | :--- |
  | `220-224` `P011` | finalidade obrigatória | **brancos** — zeros é recusa |
  | `225-226` `P013` | `CC` ou `PP`, obrigatório | **brancos** — preenchido é recusa |

  Em qualquer forma: `128-135` (`G044`) exige data válida — **anterior ou posterior à data de pagamento, tanto faz**; `136-150` (`G042`) exige valor **idêntico** ao `120-134` do Segmento A pareado, e o banco enuncia isso por extenso (_"divergente do Segmento A colunas 120 a 134"_) — preencher com valor plausível não basta; `218-219` aceita brancos **e** zeros, só código do domínio DOC recusa; `105-119` e o trailer `042-059` saem zerados, **contra** o que `G058` e `G059 'AQ'` mandam. Endereço do favorecido (`033-127`) é aceito vazio, multi-lote com formas mistas é aceito. ⚠️ **Campo sem asterisco no layout e sem código G059 é território não mapeado, não território livre** — os três campos que recusam são exatamente assim.

- **O Validador Universal trava em dois casos, e travar esconde defeito.** CEP inexistente e crítica em lote **não-primeiro** fazem a página não devolver relatório — medido por eliminação na inquiry-0033. **"Quebrou" nunca significa "arquivo válido"**, e o relatório dele é prosa sem código G059: repete a crítica por registro afetado, então contar linhas não é contar defeitos. Três das cinco críticas daquele dia **não têm código na G059** — a tabela não cobre o que o validador cobra.

- **Não há ambiente de homologação para remessa de pagamento**: a única conexão é produção, no convênio real, onde arquivo de teste vira pagamento de verdade ([ADR-0061](../../handbook/architecture/adr/0061-van-bucket-contract-supersedes-0060-pendencies.md)). O validador confere **forma** e é o mais longe que se vai sem mover dinheiro — **nunca transmita para "testar"**.
