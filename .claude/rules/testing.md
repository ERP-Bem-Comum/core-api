...
- **Segunda prova, que a inversão NÃO substitui: a suíte tem de passar DUAS VEZES seguidas, sem recriar o banco entre elas.** A ordem invertida acha dependência **entre arquivos**; ela é cega para a dependência do arquivo com o **próprio resíduo da execução anterior**, porque ambas as ordens partem de um banco limpo. Medido em 18/08/2026: `financial` passou 164/164 em ordem normal **e** invertida, e falhou **6** ao repetir sem limpar — todas em arquivos que a inversão aprovara minutos antes. Duas provas, dois defeitos diferentes; passar numa não diz nada sobre a outra.
  ...
  **Dívida quitada em 19/08/2026 (#741, #747).** A repetição sem limpar chegou a **12** falhas — cresceu enquanto arquivos novos entravam sob a mesma premissa. Corrigidas as quatro causas, `financial` passa nas **três** medições: banco limpo, repetição sem recriar, e ordem invertida sobre o banco sujo. A allowlist de `tests/cleanup/integration-rerun-safety.test.ts` está **vazia** — e é assim que ela deve permanecer.

  > ⚠️ **A contagem não é o invariante, e citá-la aqui já envelheceu duas vezes.** Eram 169 em 19/08, 175 em 21/08 e **204 em 24/08** (49 arquivos: 203 pass + 1 skip esperado, o `native-pdf REAL` que depende de fixture na pasta gitignored). Quem comparar o número contra uma medição nova conclui que sobrou coisa. O que a rule fixa é **passar nas três, com a allowlist vazia** — a suíte cresce, a propriedade não muda.

  A quarta causa não era contador de processo e vale por si: `dashboard-cost-centers` fazia `delete(finCostCenters)` **sem `WHERE`**, apagando junto o **seed das migrations 0013/0043** — que não volta, porque a migration já está aplicada. O efeito aparecia em OUTRO arquivo (`cost-center-read`, que lê o seed), um que passava sozinho e falhava depois dele. **Tabela com seed de migration limpa-se pelos ids que o arquivo inseriu, nunca inteira** — é a exceção à regra "por tabela, nunca por PK" acima, e as duas convivem porque respondem a perigos opostos: lá o risco é resíduo com outro id, aqui é apagar dado que ninguém repõe.
