...
verify:
  - claim: 'o port `Authenticator` do ADR-0024 não existe em src — nem o nome'
    ...
    expect: []
  - claim: 'o vínculo de identidade externa do ADR-0055 ainda não chegou ao schema'
    root: 'src/modules/auth/adapters/persistence'
    pattern: 'external_subject'
    expect: []
...
- **A norma vigente de autenticação é o [ADR-0055](../../handbook/architecture/adr/0055-cognito-external-idp-supersedes-0024-authn.md), não o ADR-0024 — e o 0024 não avisa isso.** O 0055 (`Accepted`) **supersede parcialmente** o 0024: caem "fonte de identidade", "emissão de token", "refresh opaco server-side" e a invariante `:128`; **todo o RBAC do 0024 segue vigente e inalterado**. ⚠️ O `Status` do ADR-0024 ainda diz só `Accepted`, **sem ponteiro para o 0055** — diferente do ADR-0056, que declara `Superseded by`. Quem abrir o 0024 direto vai ler como norma corrente algo que já caiu. Ao mexer em authN, o documento a seguir é o 0055 (verificadores isolados por `iss`, `external_subject`, vínculo JIT, as 9 invariantes de token).

- **O port `Authenticator` nunca existiu, e a premissa já se propagou para dois ADRs.** O ADR-0024 o cita quatro vezes prometendo que "um `OidcAuthenticator` pode ser plugado depois sem refactor de domínio"; não há **nenhuma** ocorrência do nome em `src/` ou `tests/`. O ADR-0055 `:63-66` **repete a premissa** ao justificar o nome da coluna: _"o ADR-0024 desenhou um port `Authenticator` abstrato…"_. ⚠️ A decisão do 0055 (coluna neutra `external_subject`, não `cognito_sub`) **continua certa** — pelo precedente `legacy_id`, citado ali mesmo —, mas não pela razão que ela invoca. Não reutilizar esse argumento em decisão nova. O que existe de OIDC-ready é uma coisa só, e é real: `password_hash` **nullable** em `auth_user`. A verificação de credencial é o **corpo** de `authenticate-user.ts`, apoiada em `PasswordHasher`, `TokenIssuer`, `RefreshTokenMinter` e `LoginLockoutStore` — quatro ports que abstraem o *como*, nenhum deles a **fonte** da autenticação. Mesma classe do read/write split do ADR-0026 que motivou a [spec 040](../../handbook/specs/040-rules-match-code-reality/spec.md): afirmação de ADR que o código nunca implementou.
